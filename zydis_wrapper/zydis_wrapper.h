...
class Zydis {
 public:
  ...
  size_t ResolveOpValue(
      int opindex,
      const std::function<size_t(ZydisRegister)>& resolveReg) const;
  bool IsBranchGoingToExecute(size_t cflags, size_t ccx) const;
  static bool IsBranchGoingToExecute(ZydisMnemonic id, size_t cflags,
                                     size_t ccx);
  bool IsConditionalGoingToExecute(size_t cflags, size_t ccx) const;
  static bool IsConditionalGoingToExecute(ZydisMnemonic id, size_t cflags,
                                          size_t ccx);
  void BytesGroup(uint8_t* prefixSize, uint8_t* opcodeSize, uint8_t* group1Size,
                  uint8_t* group2Size, uint8_t* group3Size) const;
  ...
  bool IsBranchType(/*BranchType bt*/...) const;
