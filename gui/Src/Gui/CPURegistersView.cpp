...
#define CIP PC
#define CSP SP
#define RegistersView RegistersViewA64
...
CPURegistersViewA64::CPURegistersViewA64(CPUWidget* parent)
    : RegistersView(parent), mParent(parent) {
  ...
  wCM_Modify =
      new QAction(DIcon("register_edit.png"), tr("Modify value"), this);
  wCM_Modify->setShortcut(QKeySequence(Qt::Key_Enter));
  wCM_ToggleValue =
      setupAction(DIcon("register_toggle.png"), tr("Toggle"), this);
  wCM_Undo = setupAction(DIcon("undo.png"), tr("Undo"), this);
  wCM_CopyToClipboard = setupAction(DIcon("copy.png"), tr("Copy value"), this);
  wCM_CopyFloatingPointValueToClipboard =
      setupAction(DIcon("copy.png"), tr("Copy floating point value"), this);
  wCM_CopySymbolToClipboard =
      setupAction(DIcon("pdb.png"), tr("Copy Symbol Value"), this);
  wCM_CopyAll =
      setupAction(DIcon("copy-alt.png"), tr("Copy all registers"), this);
  wCM_FollowInDisassembly =
      new QAction(DIcon(QString("processor%1.png").arg(ArchValue("32", "64"))),
                  tr("Follow in Disassembler"), this);
  wCM_FollowInDump = new QAction(DIcon("dump.png"), tr("Follow in Dump"), this);
  wCM_FollowInStack =
      new QAction(DIcon("stack.png"), tr("Follow in Stack"), this);
  // wCM_FollowInMemoryMap = new QAction(DIcon("memmap_find_address_page"),
  //                                    tr("Follow in Memory Map"), this);
  // wCM_Incrementx87Stack = setupAction(DIcon("arrow-small-down.png"),
  //                                     tr("Increment x87 Stack"), this);
  // wCM_Decrementx87Stack =
  //     setupAction(DIcon("arrow-small-up.png"), tr("Decrement x87 Stack"),
  //     this);
  wCM_ChangeFPUView =
      new QAction(DIcon("change-view.png"), tr("Change view"), this);
  wCM_IncrementPtrSize =
      setupAction(DIcon("register_inc.png"),
                  ArchValue(tr("Increase 4"), tr("Increase 8")), this);
  wCM_DecrementPtrSize =
      setupAction(DIcon("register_dec.png"),
                  ArchValue(tr("Decrease 4"), tr("Decrease 8")), this);
  ...
  mSwitchSIMDDispMode =
      new QMenu(tr("Change SIMD Register Display Mode"), this);
  ...
  // foreign messages
  connect(Bridge::getBridge(), SIGNAL(updateRegisters()), this,
          SLOT(updateRegistersSlot()));
  ...
  connect(parent->getDisasmWidget(), SIGNAL(selectionChanged(dsint)), this,
          SLOT(disasmSelectionChangedSlot(dsint)));
  // context menu actions
  connect(wCM_Increment, SIGNAL(triggered()), this, SLOT(onIncrementAction()));
  connect(wCM_ChangeFPUView, SIGNAL(triggered()), this,
          SLOT(onChangeFPUViewAction()));
  connect(wCM_Decrement, SIGNAL(triggered()), this, SLOT(onDecrementAction()));
  // connect(wCM_Incrementx87Stack, SIGNAL(triggered()), this,
  //         SLOT(onIncrementx87StackAction()));
  // connect(wCM_Decrementx87Stack, SIGNAL(triggered()), this,
  //         SLOT(onDecrementx87StackAction()));
  ...
  connect(wCM_ToggleValue, SIGNAL(triggered()), this,
          SLOT(onToggleValueAction()));
  ...
  connect(wCM_CopyFloatingPointValueToClipboard, SIGNAL(triggered()), this,
          SLOT(onCopyFloatingPointToClipboardAction()));
  connect(wCM_CopySymbolToClipboard, SIGNAL(triggered()), this,
          SLOT(onCopySymbolToClipboardAction()));
  connect(wCM_CopyAll, SIGNAL(triggered()), this, SLOT(onCopyAllAction()));
  connect(wCM_FollowInDisassembly, SIGNAL(triggered()), this,
          SLOT(onFollowInDisassembly()));
  connect(wCM_FollowInDump, SIGNAL(triggered()), this, SLOT(onFollowInDump()));
  connect(wCM_FollowInStack, SIGNAL(triggered()), this,
          SLOT(onFollowInStack()));
  // connect(wCM_FollowInMemoryMap, SIGNAL(triggered()), this,
  //        SLOT(onFollowInMemoryMap()));
  connect(wCM_IncrementPtrSize, SIGNAL(triggered()), this,
          SLOT(onIncrementPtrSize()));
  connect(wCM_DecrementPtrSize, SIGNAL(triggered()), this,
          SLOT(onDecrementPtrSize()));
...
void CPURegistersViewA64::refreshShortcutsSlot() {
  ...
  wCM_IncrementPtrSize->setShortcut(
      ConfigShortcut("ActionIncreaseRegisterPtrSize"));
  wCM_DecrementPtrSize->setShortcut(
      ConfigShortcut("ActionDecreaseRegisterPtrSize"));
...
void CPURegistersViewA64::mousePressEvent(QMouseEvent* event) {
  if (!isActive) return;
  ...
  if (event->y() < yTopSpacing - mButtonHeight) {
    ...
    if (identifyRegister(y, x, &r)) {
      ...
      if (CPUDisassemblyView->isHighlightMode()) {
        if (mGPR.contains(r) && r != FLAGS)
          CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
              ZydisTokenizer::TokenType::GeneralRegister,
              mRegisterMapping.constFind(r).value()));
        else if (mFPUMMX.contains(r))
          CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
              ZydisTokenizer::TokenType::MmxRegister,
              mRegisterMapping.constFind(r).value()));
        else if (mFPUXMM.contains(r))
          CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
              ZydisTokenizer::TokenType::XmmRegister,
              mRegisterMapping.constFind(r).value()));
        else if (mFPUYMM.contains(r))
          CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
              ZydisTokenizer::TokenType::YmmRegister,
              mRegisterMapping.constFind(r).value()));
        else if (mSEGMENTREGISTER.contains(r))
          CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
              ZydisTokenizer::TokenType::MemorySegment,
              mRegisterMapping.constFind(r).value()));
        else
          mSelected = r;
      } else
...
void CPURegistersViewA64::mouseDoubleClickEvent(QMouseEvent* event) {
  if (!isActive || event->button() != Qt::LeftButton) return;
  // get mouse position
  const int y = (event->y() - yTopSpacing) / (double)mRowHeight;
  const int x = event->x() / (double)mCharWidth;

  // do we find a corresponding register?
  if (!identifyRegister(y, x, 0)) return;
...
void CPURegistersViewA64::keyPressEvent(QKeyEvent* event) {
  if (isActive) {
    int key = event->key();
...
void CPURegistersViewA64::debugStateChangedSlot(DBGSTATE state) {
  if (state == stopped) {
    updateRegistersSlot();
    isActive = false;
  } else {
...
void CPURegistersViewA64::updateRegistersSlot() {
  // read registers
  REGDUMP* z = DbgGetRegDumpEx(nullptr, 0);
  // update gui
  setRegisters(z);
}

void CPURegistersViewA64::ModifyFields(const QString& title,
                                    STRING_VALUE_TABLE_t* table, SIZE_T size) {
  ...
  for (i = 0; i < size; i++)
    items << QApplication::translate("RegistersView_ConstantsOfRegisters",
                                     table[i].string) +
                 QString(" (%1)").arg(table[i].value, 0, 16);

  mQListWidget->addItems(items);

  mSelectFields.setWindowTitle(title);
  if (mSelectFields.exec() != QDialog::Accepted) return;

  if (mQListWidget->selectedItems().count() != 1) return;
  ...
  for (i = 0; i < size; i++) {
    if (QApplication::translate("RegistersView_ConstantsOfRegisters",
                                table[i].string) +
            QString(" (%1)").arg(table[i].value, 0, 16) ==
        itemText)
      break;
...
#define MODIFY_FIELDS_DISPLAY(prefix, title, table)  \
  ModifyFields(prefix + QChar(' ') + QString(title), \
               (STRING_VALUE_TABLE_t*)&table, SIZE_TABLE(table))

/**
 * @brief   This function displays the appropriate edit dialog according to
 * selected register
 * @return  Nothing.
 */

void CPURegistersViewA64::displayEditDialog() {
  if (mFPU.contains(mSelected)) {
    if (mTAGWORD.contains(mSelected))
      MODIFY_FIELDS_DISPLAY(
          tr("Edit"), "Tag " + mRegisterMapping.constFind(mSelected).value(),
          TagWordValueStringTable);
#if 0
      ...
      mEditFloat.setWindowTitle(tr("Edit NEON register"));
      mEditFloat.loadData(registerValue(&wRegDumpStruct, mSelected));
      mEditFloat.show();
      mEditFloat.selectAllText();
      if (mEditFloat.exec() == QDialog::Accepted)
      ...
      mLineEdit.setText(GetRegStringValueFromValue(
      ...
      if (sizeRegister == 10) mLineEdit.setFpuMode();
      ...
      do {
        ...
        if (mLineEdit.exec() != QDialog::Accepted)
          ...
          if (mUSHORTDISPLAY.contains(mSelected))
            ...
            if (sizeRegister == 10 &&
                (mLineEdit.editText.contains(QChar('.')) ||
                 editTextLower == "nan" || editTextLower == "inf" ||
              ...
              if (pArray.size() == sizeRegister * 2) {
                ...
                if (pData != NULL) {
                  ...
                  if (ok) {
                    if (!ConfigBool(
                            "Gui",
                            "FpuRegistersLittleEndian"))  // reverse byte order
                                                          // if it is big-endian
                    {
                      pArray = ByteReverse(QByteArray(pData, sizeRegister));
                      setRegister(mSelected,
                                  reinterpret_cast<duint>(pArray.constData()));
                    } else
          ...
          if (!ok) {
            ...
            SimpleWarningBox(this, tr("ERROR CONVERTING TO HEX"),
                             tr("ERROR CONVERTING TO HEX"));
          } else
...
void CPURegistersViewA64::CreateDumpNMenu(QMenu* dumpMenu) {
  QList<QString> names;
...
void CPURegistersViewA64::onIncrementx87StackAction() {
#if 0
...
void CPURegistersViewA64::onDecrementx87StackAction() {
#if 0
    if(mFPUx87_80BITSDISPLAY.contains(mSelected))
        setRegister(x87SW_TOP, ((* ((duint*) registerValue(&wRegDumpStruct, x87SW_TOP))) - 1) % 8);
#endif
...
void CPURegistersViewA64::onIncrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) + 1);
}

void CPURegistersViewA64::onDecrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - 1);
}

void CPURegistersViewA64::onIncrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
        (*((duint*)registerValue(&wRegDumpStruct, mSelected))) + sizeof(void*));
}

void CPURegistersViewA64::onDecrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
        (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - sizeof(void*));
...
void CPURegistersViewA64::onPushAction() {
...
void CPURegistersViewA64::onPopAction() {
...
void CPURegistersViewA64::onZeroAction() {
  if (mSETONEZEROTOGGLE.contains(mSelected)) {
    // if(mSelected >= x87r0 && mSelected <= x87r7 || mSelected >= x87st0 &&
    // mSelected <= x87st7)
    //    setRegister(mSelected, reinterpret_cast<duint>("\0\0\0\0\0\0\0\0\0"));
    //    //9 zeros and 1 terminating zero
    // else
    setRegister(mSelected, 0);
  }
}

void CPURegistersViewA64::onSetToOneAction() {
  if (mSETONEZEROTOGGLE.contains(mSelected)) {
    // if(mSelected >= x87r0 && mSelected <= x87r7 || mSelected >= x87st0 &&
    // mSelected <= x87st7)
    //    setRegister(mSelected,
    //    reinterpret_cast<duint>("\0\0\0\0\0\0\0\x80\xFF\x3F"));
    // else
    setRegister(mSelected, 1);
  }
}

void CPURegistersViewA64::onModifyAction() {
  if (mMODIFYDISPLAY.contains(mSelected)) displayEditDialog();
}

void CPURegistersViewA64::onToggleValueAction() {
...
void CPURegistersViewA64::onUndoAction() {
  if (mUNDODISPLAY.contains(mSelected)) {
    if (mFPUMMX.contains(mSelected) || mFPUXMM.contains(mSelected) ||
        mFPUYMM.contains(mSelected) ||
        mFPUx87_80BITSDISPLAY.contains(mSelected))
      setRegister(mSelected,
                  (duint)registerValue(&wCipRegDumpStruct, mSelected));
    else
      setRegister(mSelected,
                  *(duint*)registerValue(&wCipRegDumpStruct, mSelected));
  }
...
void CPURegistersViewA64::onCopyToClipboardAction() {
  Bridge::CopyToClipboard(GetRegStringValueFromValue(
      mSelected, registerValue(&wRegDumpStruct, mSelected)));
}

void CPURegistersViewA64::onCopyFloatingPointToClipboardAction() {
  Bridge::CopyToClipboard(ToLongDoubleString(
      ((X87FPUREGISTER*)registerValue(&wRegDumpStruct, mSelected))->data));
}

void CPURegistersViewA64::onCopySymbolToClipboardAction() {
  if (mLABELDISPLAY.contains(mSelected)) {
    QString symbol = getRegisterLabel(mSelected);
    if (symbol != "") Bridge::CopyToClipboard(symbol);
  }
}

void CPURegistersViewA64::onHighlightSlot() {
  Disassembly* CPUDisassemblyView = mParent->getDisasmWidget();
  if (mGPR.contains(mSelected) && mSelected != FLAGS)
    CPUDisassemblyView->hightlightToken(ZydisTokenizer::SingleToken(
        ZydisTokenizer::TokenType::GeneralRegister,
        mRegisterMapping.constFind(mSelected).value()));
...
void CPURegistersViewA64::onFollowInDisassembly() {
...
void CPURegistersViewA64::onFollowInDump() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewA64::onFollowInDumpN() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewA64::onFollowInStack() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewA64::onFollowInMemoryMap() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewA64::displayCustomContextMenuSlot(QPoint pos) {
  if (!isActive) return;
  QMenu wMenu(this);
  ...
  if (mSelected != UNKNOWN) {
    ...
    if (mLABELDISPLAY.contains(mSelected)) {
      QString symbol = getRegisterLabel(mSelected);
      if (symbol != "") wMenu.addAction(wCM_CopySymbolToClipboard);
    }

    if ((mGPR.contains(mSelected) && mSelected != FLAGS) ||
        mSEGMENTREGISTER.contains(mSelected) || mFPUMMX.contains(mSelected) ||
        mFPUXMM.contains(mSelected) || mFPUYMM.contains(mSelected)) {
      wMenu.addAction(wCM_Highlight);
    }

    if (mUNDODISPLAY.contains(mSelected) &&
        CompareRegisters(mSelected, &wRegDumpStruct, &wCipRegDumpStruct) != 0) {
    ...
    if (mFPUMMX.contains(mSelected) ||
        mFPUx87_80BITSDISPLAY.contains(mSelected)) {
      // if (mFpuMode != 0) wMenu.addAction(mDisplaySTX);
      // if (mFpuMode != 1) wMenu.addAction(mDisplayx87rX);
      if (mFpuMode != 2) wMenu.addAction(mDisplayMMX);
    }
...
#if 0
    wMenu.addSeparator();
    QAction* wHwbpCsp = wMenu.addAction(
        DIcon("breakpoint.png"),
        tr("Set Hardware Breakpoint on %1").arg(ArchValue("ESP", "RSP")));
#endif
    QAction* wAction = wMenu.exec(this->mapToGlobal(pos));

    // if (wAction == wHwbpCsp) DbgCmdExec("bphws csp,rw");
  }
}

void CPURegistersViewA64::onSIMDMode() {
  Config()->setUint("Gui", "SIMDRegistersDisplayMode",
                    dynamic_cast<QAction*>(sender())->data().toInt());
  emit refresh();
  GuiUpdateDisassemblyView();  // refresh display mode for data in disassembly
}

void CPURegistersViewA64::onFpuMode() {
  mFpuMode = (char)(dynamic_cast<QAction*>(sender())->data().toInt());
  InitMappings();
  emit refresh();
}

void CPURegistersViewA64::setRegister(REGISTER_NAME reg, duint value) {
...
void CPURegistersViewA64::disasmSelectionChangedSlot(dsint va) {
  mHighlightRegs =
      mParent->getDisasmWidget()
          ->DisassembleAt(va - mParent->getDisasmWidget()->getBase())
          .regsReferenced;
  emit refresh();
}
