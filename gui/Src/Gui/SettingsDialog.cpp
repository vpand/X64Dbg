...
SettingsDialog::SettingsDialog(QWidget* parent)
  ...
  ui->setupUi(this);
  ui->comboBoxDefPlatform->addItem("Local macOS/Simulator/Linux");
  ui->comboBoxDefPlatform->addItem("Remote iOS");
  ui->comboBoxDefPlatform->addItem("Remote Android");
  ui->comboBoxDefPlatform->addItem("Local VP iOS Simulator");
  ui->comboBoxDefPlatform->addItem("Remote VP Android Emulator");
  ui->comboBoxDefPlatform->addItem("Local UraniumVM");
  ui->comboBoxDefPlatform->addItem("Remote UraniumVM iOS");
  ui->comboBoxDefPlatform->addItem("Remote UraniumVM Android");
  // set window flags
...
void SettingsDialog::LoadSettings() {
  ...
  settings.dbgDefaultPlatform = pt_default_platform;
  settings.dbgRemoteAndroidPort = remote_default_port;
  settings.dbgRemoteAndroidIP = &realDbgRemoteAndroidIP;
  settings.dbgUSBAndroid = &realDbgUSBAndroid;
  settings.dbgRemoteiOSPort = remote_default_port;
  settings.dbgRemoteiOSIP = &realDbgRemoteiOSIP;
  settings.dbgUSBiOS = &realDbgUSBiOS;
  ...
  char* setvalue = &exceptionRange[0];
  GetSettingBool(DBGSETTING_SECTION, DSSKEY_CLOUD_PLATFORM,
                 &settings.dbgCloudPlatform);
  if (BridgeSettingGet(DBGSETTING_SECTION, DSSKEY_IOS_IP, setvalue)) {
    realDbgRemoteiOSIP = setvalue;
  }
  if (BridgeSettingGet(DBGSETTING_SECTION, DSSKEY_ANDROID_IP, setvalue) &&
      isdigit(setvalue[0])) {
    realDbgRemoteAndroidIP = setvalue;
  } else {
    realDbgRemoteAndroidIP = "127.0.0.1";
  }
  if (BridgeSettingGet(DBGSETTING_SECTION, DSSKEY_IOS_USB, setvalue)) {
    realDbgUSBiOS = setvalue;
  }
  if (BridgeSettingGet(DBGSETTING_SECTION, DSSKEY_ANDROID_USB, setvalue)) {
    realDbgUSBAndroid = setvalue;
  }
  BridgeSettingGetUint(DBGSETTING_SECTION, DSSKEY_ANDROID_PORT,
                       &settings.dbgRemoteAndroidPort);
  BridgeSettingGetUint(DBGSETTING_SECTION, DSSKEY_IOS_PORT,
                       &settings.dbgRemoteiOSPort);
  BridgeSettingGetUint(DBGSETTING_SECTION, DSSKEY_DEFAULT_PLATFORM,
                       &settings.dbgDefaultPlatform);
  oldDbgDefaultPlatform = settings.dbgDefaultPlatform;

  QRegExp rxpip(
      "\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4]["
      "0-9]|[01]?[0-9][0-9]?)\\b");
  QRegExp rxpport(
      "^([0-9]|[1-9]\\d|[1-9]\\d{2}|[1-9]\\d{3}|[1-5]\\d{4}|6[0-4]\\d{3}|65[0-"
      "4]\\d{2}|655[0-2]\\d|6553[0-5])$");
  QRegExpValidator* valip = new QRegExpValidator(rxpip, this);
  QRegExpValidator* valport = new QRegExpValidator(rxpport, this);
  ui->lineEditAndroidIP->setValidator(valip);
  ui->lineEditAndroidIP->setText(*settings.dbgRemoteAndroidIP);
  ui->checkBoxCloud->setChecked(settings.dbgCloudPlatform);
  ui->lineEditAndroidPort->setValidator(valport);
  ui->lineEditAndroidPort->setText(
      QString("%1").arg(settings.dbgRemoteAndroidPort));
  ui->lineEditIOSIP->setValidator(valip);
  ui->lineEditIOSIP->setText(*settings.dbgRemoteiOSIP);
  ui->lineEditIOSPort->setValidator(valport);
  ui->lineEditIOSPort->setText(QString("%1").arg(settings.dbgRemoteiOSPort));
  ui->labelDbgsvrPort->setText(
      QString("%1").arg(settings.dbgRemoteiOSPort + 1));
  ui->comboBoxDefPlatform->setCurrentIndex(settings.dbgDefaultPlatform);
  ui->lineEditADB->setText(*settings.dbgUSBAndroid);
  ui->lineEditMux->setText(*settings.dbgUSBiOS);
}

void SettingsDialog::SaveSettings() {
  // Debugger tab
  realDbgRemoteAndroidIP = ui->lineEditAndroidIP->text();
  realDbgUSBAndroid = ui->lineEditADB->text();
  realDbgRemoteiOSIP = ui->lineEditIOSIP->text();
  realDbgUSBiOS = ui->lineEditMux->text();
  settings.dbgRemoteAndroidPort = ui->lineEditAndroidPort->text().toInt();
  settings.dbgRemoteiOSPort = ui->lineEditIOSPort->text().toInt();
  ...
  BridgeSettingSetUint(DBGSETTING_SECTION, DSSKEY_DEFAULT_PLATFORM,
                       settings.dbgDefaultPlatform);
  BridgeSettingSetUint(DBGSETTING_SECTION, DSSKEY_ANDROID_PORT,
                       settings.dbgRemoteAndroidPort);
  BridgeSettingSetUint(DBGSETTING_SECTION, DSSKEY_IOS_PORT,
                       settings.dbgRemoteiOSPort);
  BridgeSettingSet(DBGSETTING_SECTION, DSSKEY_ANDROID_IP,
                   settings.dbgRemoteAndroidIP->toUtf8().data());
  BridgeSettingSet(DBGSETTING_SECTION, DSSKEY_ANDROID_USB,
                   settings.dbgUSBAndroid->toUtf8().data());
  BridgeSettingSet(DBGSETTING_SECTION, DSSKEY_IOS_IP,
                   settings.dbgRemoteiOSIP->toUtf8().data());
  BridgeSettingSet(DBGSETTING_SECTION, DSSKEY_IOS_USB,
                   settings.dbgUSBiOS->toUtf8().data());
  BridgeSettingSetUint(DBGSETTING_SECTION, DSSKEY_CLOUD_PLATFORM, settings.dbgCloudPlatform);
...
void SettingsDialog::on_pushButtonADB_clicked() {
  ...
  realDbgUSBAndroid = filename;
  ui->lineEditADB->setText(filename);
}

void SettingsDialog::on_pushButtonMux_clicked() {
  auto filename =
  ...
  filename = QDir::toNativeSeparators(
      filename);  // convert to native path format (with backlashes)
  realDbgUSBiOS = filename;
  ui->lineEditMux->setText(filename);
}
...
void SettingsDialog::on_comboBoxDefPlatform_currentIndexChanged(int index) {
  settings.dbgDefaultPlatform = index;
}

void SettingsDialog::on_lineEditIOSPort_textChanged(const QString& text) {
  int port = ui->lineEditIOSPort->text().toInt();
  ui->labelDbgsvrPort->setText(QString("%1").arg(port + 1));
}
