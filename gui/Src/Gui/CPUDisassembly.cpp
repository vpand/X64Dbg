...
void CPUDisassembly::setupRightClickContextMenu() {
  ...
  mMenuBuilder->addAction(
      makeShortcutAction(DIcon("freeze.png"), tr("Show ExecMap"),
                         SLOT(showExecMapSlot()), "ActionShowExecMap"));
  mMenuBuilder->addSeparator();
...
void CPUDisassembly::findCommandSlot() {
  ...
  if (refFindType != -1) {
    size = 0; // let findasm command to determine the section size
...
void CPUDisassembly::showExecMapSlot() {
  mCPUWin->showExecMap();
}
