...
MHTabWidget::MHTabWidget(QWidget* parent, bool allowDetach, bool allowDelete)
  ...
  connect(mTabBar, SIGNAL(OnDetachTab(int, const QPoint&)), this,
          SLOT(DetachTab(int, const QPoint&)));
...
MHTabWidget::~MHTabWidget() {
  disconnect(mTabBar, SIGNAL(OnMoveTab(int, int)), this,
             SLOT(MoveTab(int, int)));
  disconnect(mTabBar, SIGNAL(OnDetachTab(int, const QPoint&)), this,
             SLOT(DetachTab(int, const QPoint&)));
  disconnect(mTabBar, SIGNAL(OnDeleteTab(int)), this, SLOT(DeleteTab(int)));
  disconnect(mTabBar, SIGNAL(currentChanged(int)), this,
             SLOT(currentChanged(int)));
  delete mTabBar;
}
...
QWidget* MHTabWidget::widget(int index) const {
  ...
  if (index < baseCount) return QTabWidget::widget(index);

  // Otherwise it's going to be a window
  return mWindows.at(index - baseCount);
}

int MHTabWidget::count() const { return QTabWidget::count() + mWindows.size(); }

QList<QWidget*> MHTabWidget::windows() { return mWindows; }

// Add a tab
int MHTabWidget::addTabEx(QWidget* widget, const QIcon& icon,
                          const QString& label, const QString& nativeName) {
...
void MHTabWidget::AttachTab(QWidget* parent) {
  // Retrieve widget
  MHDetachedWindow* detachedWidget =
      reinterpret_cast<MHDetachedWindow*>(parent);
  QWidget* tearOffWidget = detachedWidget->centralWidget();

  // Reattach the tab
  addTabEx(tearOffWidget, detachedWidget->windowIcon(),
           detachedWidget->windowTitle(), detachedWidget->mNativeName);
  ...
  // Cleanup Window
  disconnect(detachedWidget, SIGNAL(OnClose(QWidget*)), this,
             SLOT(AttachTab(QWidget*)));
  disconnect(detachedWidget, SIGNAL(OnFocused(QWidget*)), this,
             SLOT(OnDetachFocused(QWidget*)));
  detachedWidget->hide();
...
void MHTabWidget::DetachTab(int index, const QPoint& dropPoint) {
  ...
  connect(detachedWidget, SIGNAL(OnClose(QWidget*)), this,
          SLOT(AttachTab(QWidget*)));
  connect(detachedWidget, SIGNAL(OnFocused(QWidget*)), this,
          SLOT(OnDetachFocused(QWidget*)));
...
void MHTabWidget::currentChanged(int index) { setLatestFocused(widget(index)); }
...
void MHTabWidget::setCurrentIndex(int index) {
  ...
  if (index < QTabWidget::count()) {
    ...
    MHDetachedWindow* window =
        dynamic_cast<MHDetachedWindow*>(widget(index)->parent());
...
MHTabBar* MHTabWidget::tabBar() const { return mTabBar; }

const QList<MIDPKey>& MHTabWidget::MIDP_getItems() const { return mHistory; }
...
void MHTabWidget::MIDP_selected(MIDPKey index) {
  ...
  MHDetachedWindow* window = dynamic_cast<MHDetachedWindow*>(
      reinterpret_cast<QWidget*>(index)->parent());
  window->activateWindow();
  window->showNormal();
  window->setFocus();
}
...
QString MHTabWidget::getNativeName(int index) {
  if (index < QTabWidget::count()) {
    ...
    MHDetachedWindow* window = dynamic_cast<MHDetachedWindow*>(
        mWindows.at(index - QTabWidget::count())->parent());
    if (window)
...
void MHTabWidget::showPreviousView() { mHistoryPopup->gotoPreviousItem(); }

void MHTabWidget::showNextView() { mHistoryPopup->gotoNextItem(); }
