...
// turn off folding currently
// failed this function to disable code folding
#define DbgFunctionGet(...) false

CPUSideBar::CPUSideBar(CPUDisassembly* Ptr, QWidget* parent)
...
CPUSideBar::~CPUSideBar() { delete mFontMetrics; }
...
void CPUSideBar::updateSlots() {
  ...
  connect(Bridge::getBridge(), SIGNAL(foldDisassembly(duint, duint)), this,
          SLOT(foldDisassembly(duint, duint)));
...
void CPUSideBar::updateColors() {
  ...
  mConditionalJumpLineFalseColor =
      ConfigColor("SideBarConditionalJumpLineFalseColor");
  mUnconditionalJumpLineFalseColor =
      ConfigColor("SideBarUnconditionalJumpLineFalseColor");
  mConditionalJumpLineTrueColor =
      ConfigColor("SideBarConditionalJumpLineTrueColor");
  mUnconditionalJumpLineTrueColor =
      ConfigColor("SideBarUnconditionalJumpLineTrueColor");
  mConditionalJumpLineFalseBackwardsColor =
      ConfigColor("SideBarConditionalJumpLineFalseBackwardsColor");
  mUnconditionalJumpLineFalseBackwardsColor =
      ConfigColor("SideBarUnconditionalJumpLineFalseBackwardsColor");
  mConditionalJumpLineTrueBackwardsColor =
      ConfigColor("SideBarConditionalJumpLineTrueBackwardsColor");
  mUnconditionalJumpLineTrueBackwardsColor =
      ConfigColor("SideBarUnconditionalJumpLineTrueBackwardsColor");
  ...
  mBulletDisabledBreakpointColor =
      ConfigColor("SideBarBulletDisabledBreakpointColor");
  ...
  mUnconditionalBackwardsPen =
      QPen(mUnconditionalJumpLineFalseBackwardsColor, 1, Qt::SolidLine);
  mConditionalBackwardsPen =
      QPen(mConditionalJumpLineFalseBackwardsColor, 1, Qt::DashLine);
}
...
void CPUSideBar::changeTopmostAddress(dsint i) {
  topVA = i;
  regDump = DbgGetRegDumpEx(nullptr, 0);
  reload();
}

void CPUSideBar::setViewableRows(int rows) { viewableRows = rows; }
...
bool CPUSideBar::isJump(int i) const {
  if (i < 0 || i >= mInstrBuffer->size()) return false;

  const Instruction_t& instr = mInstrBuffer->at(i);
  Instruction_t::BranchType branchType = instr.branchType;
  if (branchType == Instruction_t::Unconditional ||
      branchType == Instruction_t::Conditional) {
    duint start = mDisas->getBase();
    duint end = start + mDisas->getSize();
    duint addr =
        instr.branchDestination;  // DbgGetBranchDestination(start + instr.rva);

    if (!addr) return false;

    return addr >= start &&
           addr < end;  // do not draw jumps that go out of the section
  }
...
void CPUSideBar::paintEvent(QPaintEvent* event) {
  ...
  if (mInstrBuffer->size() == 0) return;

  duint pc = regPC();
  if (mCodeFoldingManager.isFolded(pc)) {
    mCodeFoldingManager.expandFoldSegment(pc);
    mDisas->reloadData();
  }
  ...
  auto appendReg = [&regLabel, last_va, first_va](const QString& name,
                                                  duint value) {
  ...
  if (!debugee) return;
  auto arch = debugee->db->info.arch;
  if (arch == mana::X86_64) {
    const REGDUMPX64& reg = *(REGDUMPX64*)regDump;
    appendReg("RIP", reg.regcontext.cip);
    appendReg("RAX", reg.regcontext.cax);
    appendReg("RCX", reg.regcontext.ccx);
    appendReg("RDX", reg.regcontext.cdx);
    appendReg("RBX", reg.regcontext.cbx);
    appendReg("RSP", reg.regcontext.csp);
    appendReg("RBP", reg.regcontext.cbp);
    appendReg("RSI", reg.regcontext.csi);
    appendReg("RDI", reg.regcontext.cdi);
    appendReg("R8", reg.regcontext.r8);
    appendReg("R9", reg.regcontext.r9);
    appendReg("R10", reg.regcontext.r10);
    appendReg("R11", reg.regcontext.r11);
    appendReg("R12", reg.regcontext.r12);
    appendReg("R13", reg.regcontext.r13);
    appendReg("R14", reg.regcontext.r14);
    appendReg("R15", reg.regcontext.r15);
  } else if (arch == mana::ARM64) {
    const REGDUMPA64& reg = *(REGDUMPA64*)regDump;
    for (int i = 0; i < 30; i++) {
      char regname[16];
      sprintf(regname, "x%d", i);
      appendReg(regname, reg.regcontext.gpr[i]);
    }
    appendReg("lr", reg.regcontext.clr);
    appendReg("sp", reg.regcontext.csp);
    appendReg("pc", reg.regcontext.cip);
  } else if (arch == mana::ARM) {
    const REGDUMPA32& reg = *(REGDUMPA32*)regDump;
    for (int i = 0; i < 13; i++) {
      char regname[16];
    ...
    const REGDUMPX64& reg = *(REGDUMPX64*)regDump;
    appendReg("EIP", reg.regcontext.cip);
    appendReg("EAX", reg.regcontext.cax);
    appendReg("ECX", reg.regcontext.ccx);
    appendReg("EDX", reg.regcontext.cdx);
    appendReg("EBX", reg.regcontext.cbx);
    appendReg("ESP", reg.regcontext.csp);
    appendReg("EBP", reg.regcontext.cbp);
    appendReg("ESI", reg.regcontext.csi);
    appendReg("EDI", reg.regcontext.cdi);
  }
  if (ConfigBool("Gui", "SidebarWatchLabels")) {
    BridgeList<WATCHINFO> WatchList;
    DbgGetWatchList(&WatchList);
    for (int i = 0; i < WatchList.Count(); i++) {
      if (WatchList[i].varType == WATCHVARTYPE::TYPE_UINT ||
          WatchList[i].varType == WATCHVARTYPE::TYPE_ASCII ||
          WatchList[i].varType == WATCHVARTYPE::TYPE_UNICODE) {
  ...
  for (int line = 0; line < viewableRows; line++) {
    if (line >=
        mInstrBuffer->size())  // at the end of the page it will crash otherwise
      break;
    ...
    drawBullets(&painter, line, DbgGetBpxTypeAt(instrVA) != bp_none,
                DbgIsBpDisabled(instrVA), DbgGetBookmarkAt(nullptr, instrVA));

    if (isJump(line))  // handle jumps
    {
      duint baseAddr = mDisas->getBase();
      duint destVA = DbgGetBranchDestination(debugee, baseAddr + instr.rva);
      ...
      /*
      if(mDisas->currentEIP() != mInstrBuffer->at(line).rva) //create a setting
      for this isJumpGoingToExecute=false;
      */

      jumpoffset++;

      // Do not draw jumps that leave the memory range
      if (destVA >= mDisas->getBase() + mDisas->getSize() ||
          destVA < mDisas->getBase())
        continue;
    ...
    if (isFoldingGraphicsPresent(line) == 1) {
      if (mCodeFoldingManager.isFoldStart(instrVA))
        drawFoldingCheckbox(&painter, line * fontHeight,
                            mCodeFoldingManager.isFolded(instrVA));
      else
        drawFoldingCheckbox(&painter, line * fontHeight, false);
    } else if (mCodeFoldingManager.isFoldBody(instrVA)) {
      painter.setPen(QColor(Qt::black));
      painter.drawLine(QPointF(viewport()->width() - fontHeight / 2 -
                                   mBulletXOffset - mBulletRadius,
                               line * fontHeight),
                       QPointF(viewport()->width() - fontHeight / 2 -
                                   mBulletXOffset - mBulletRadius,
                               (line + 1) * fontHeight));
      if (mCodeFoldingManager.isFoldEnd(instrVA + instr.length - 1))
        painter.drawLine(QPointF(viewport()->width() - fontHeight / 2 -
                                     mBulletXOffset - mBulletRadius,
                                 (line + 1) * fontHeight),
                         QPointF(viewport()->width(), (line + 1) * fontHeight));
    }
  }
  if (jumpLines.size()) {
    AllocateJumpOffsets(jumpLines, labelArrows);
    for (auto i : jumpLines)
      drawJump(&painter, i.line, i.destLine, i.jumpOffset, i.isConditional,
               i.isJumpGoingToExecute, i.isSelected);
  } else {
    for (auto i = labelArrows.begin(); i != labelArrows.end(); i++)
      i->endX =
          viewport()->width() - 1 - 11 -
          (isFoldingGraphicsPresent(i->line) != 0 ? mBulletRadius + fontHeight
                                                  : 0);
  }
...
void CPUSideBar::mouseReleaseEvent(QMouseEvent* e) {
  ...
  if (line >= mInstrBuffer->size()) return;
  ...
  if (x > bulletX + bulletRadius) return;
  // calculate virtual address of clicked line
  duint wVA = mInstrBuffer->at(line).rva + mDisas->getBase();
  if (CheckBoxPresent) {
    if (x > width - fontHeight - mBulletXOffset - mBulletRadius &&
        x < width - mBulletXOffset - mBulletRadius) {
      if (e->button() == Qt::LeftButton) {
        ...
        if (mCodeFoldingManager.isFoldStart(wVA)) {
          mCodeFoldingManager.setFolded(wVA,
                                        !mCodeFoldingManager.isFolded(wVA));
        } else if (instr->rva == SelectionStart &&
                   (SelectionEnd - SelectionStart + 1) != instr->length) {
          bool success = mCodeFoldingManager.addFoldSegment(
              wVA, mDisas->getSelectionEnd() - mDisas->getSelectionStart());
          if (!success)
            GuiAddLogMessage(
                tr("Cannot fold selection.\n").toUtf8().constData());
        } else if ((DbgArgumentGet(wVA, &start, &end) ||
                    DbgFunctionGet(wVA, &start, &end)) &&
                   wVA == start) {
  ...
  if (x < bulletX - mBulletRadius) return;
...
void CPUSideBar::mouseDoubleClickEvent(QMouseEvent* event) {
  ...
  if (CheckBoxPresent) {
    if (event->x() > width() - fontHeight - mBulletXOffset - mBulletRadius &&
        event->x() < width() - mBulletXOffset - mBulletRadius) {
      if (event->button() == Qt::LeftButton) {
        duint wVA = mInstrBuffer->at(line).rva + mDisas->getBase();
        duint start = mCodeFoldingManager.getFoldBegin(wVA);
        duint end = mCodeFoldingManager.getFoldEnd(wVA);
        duint pc = regPC();
        if (mCodeFoldingManager.isFolded(wVA) || (start <= pc && end >= pc)) {
...
void CPUSideBar::mouseMoveEvent(QMouseEvent* event) {
  ...
  // calculate virtual address of clicked line
  duint wVA = 0;
  if (lineInBounds) wVA = mInstrBuffer->at(mLine).rva + mDisas->getBase();

  // check if mouse is on a code folding box
  if (mousePos.x() > width - fontHeight - mBulletXOffset - mBulletRadius &&
      mousePos.x() < width - mBulletXOffset - mBulletRadius) {
    if (isFoldingGraphicsPresent(mLine)) {
      if (mCodeFoldingManager.isFolded(wVA)) {
        QToolTip::showText(globalMousePos,
                           tr("Click to unfold, right click to delete."));
      } else {
        if (mCodeFoldingManager.isFoldStart(wVA))
          QToolTip::showText(globalMousePos,
                             tr("Click to fold, right click to delete."));
        else
  ...
  else if ((mouseBulletXOffset > mBulletRadius ||
            mouseBulletYOffset > mBulletRadius) ||
           !lineInBounds) {
    QToolTip::hideText();
    setCursor(QCursor(Qt::ArrowCursor));
  } else {
...
void CPUSideBar::drawJump(QPainter* painter, int startLine, int endLine,
                          int jumpoffset, bool conditional, bool isexecute,
                          bool isactive) {
  painter->save();
  ...
  if (!isFoldingGraphicsPresent(startLine) != 0)
    ...
    painter->drawLine(x_right - mBulletRadius - fontHeight, y_start, x,
                      y_start);
  ...
  if (!isactive)  // selected
  {
    ...
    if (endLine == viewableRows + 6) {
      ...
      if (isFoldingGraphicsPresent(endLine) != 0)
        drawStraightArrow(painter, x, y_end,
                          x_right - mBulletRadius - fontHeight, y_end);
      else
...
void CPUSideBar::drawBullets(QPainter* painter, int line, bool isbp,
                             bool isbpdisabled, bool isbookmark) {
  painter->save();
...
CPUSideBar::LabelArrow CPUSideBar::drawLabel(QPainter* painter, int Line,
                                             const QString& Text) {
  painter->save();
  ...
  drawStraightArrow(painter, rect.right() + 2, y, this->viewport()->width() - x
  - 11 - (isFoldingGraphicsPresent(Line) != 0 ? mBulletRadius + fontHeight : 0),
  y);*/
...
void CPUSideBar::drawLabelArrows(QPainter* painter,
                                 const std::vector<LabelArrow>& labelArrows) {
...
void CPUSideBar::drawFoldingCheckbox(QPainter* painter, int y, bool state) {
  ...
  painter->drawLine(QPointF(x + fontHeight / 4, y + fontHeight / 2),
                    QPointF(x + 3 * fontHeight / 4, y + fontHeight / 2));
  if (state)  // "+"
    painter->drawLine(QPointF(x + fontHeight / 2, y + fontHeight / 4),
                      QPointF(x + fontHeight / 2, y + 3 * fontHeight / 4));
  painter->restore();
}

void CPUSideBar::drawStraightArrow(QPainter* painter, int x1, int y1, int x2,
                                   int y2) {
  ...
  painter->drawLine(x2 - 1, y2 - 1, x2 - ArrowSizeX,
                    y2 - ArrowSizeY);  // Arrow top
  painter->drawLine(x2 - 1, y2, x2 - ArrowSizeX,
                    y2 + ArrowSizeY - 1);  // Arrow bottom
}

void CPUSideBar::AllocateJumpOffsets(std::vector<JumpLine>& jumpLines,
                                     std::vector<LabelArrow>& labelArrows) {
  unsigned int* numLines =
      new unsigned int[viewableRows *
                       2];  // Low:jump offsets of the vertical jumping line,
                            // High:jump offsets of the horizontal jumping line.
  ...
  std::sort(jumpLines.begin(), jumpLines.end(),
            [](const JumpLine& op1, const JumpLine& op2) {
  ...
  for (size_t i = 0; i < jumpLines.size(); i++) {
    ...
    if (jmp.line < jmp.destLine) {
      for (int j = jmp.line; j <= jmp.destLine && j < viewableRows; j++) {
        if (numLines[j] > maxJmpOffset) maxJmpOffset = numLines[j];
      }
    } else {
  ...
  for (auto i = labelArrows.begin(); i != labelArrows.end(); i++) {
    if (numLines[i->line + viewableRows] != 0)
      i->endX =
          viewportWidth - numLines[i->line + viewableRows] * JumpPadding - 15 -
          fontHeight;  // This expression should be consistent with drawJump
    else
      i->endX =
          viewportWidth - 1 - 11 -
          (isFoldingGraphicsPresent(i->line) != 0 ? mBulletRadius + fontHeight
...
int CPUSideBar::isFoldingGraphicsPresent(int line) {
  if (line < 0 ||
      line >= mInstrBuffer
                  ->size())  // There couldn't be a folding box out of viewport
    return 0;
  const Instruction_t* instr = &mInstrBuffer->at(line);
  ...
  const dsint SelectionEnd = mDisas->getSelectionEnd();
  duint start, end;
  if ((DbgArgumentGet(wVA, &start, &end) ||
       DbgFunctionGet(wVA, &start, &end)) &&
      wVA == start) {
    return end - start + 1 != instr->length ? 1 : 0;
  } else if (instr->rva >= duint(SelectionStart) &&
             instr->rva < duint(SelectionEnd)) {
...
void CPUSideBar::operator delete(void* p) { _aligned_free(p); }
