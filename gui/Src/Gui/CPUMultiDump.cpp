...
CPUMultiDump::CPUMultiDump(CPUDisassembly* disas, int nbCpuDumpTabs,
                           QWidget* parent)
  ...
  mSourceView = new SourceView("", 0, this);
  this->addTabEx(mSourceView, DIcon("source.png"), tr("PseudoCode"), "PseudoCode");
  ...
  connect(Bridge::getBridge(), SIGNAL(loadPseudoSource(const QString &, duint)), this,
          SLOT(loadPseudoSourceSlot(const QString &, duint)));
...
CPUDump* CPUMultiDump::getCurrentCPUDump() { return mCurrentCPUDump; }
...
int CPUMultiDump::getMaxCPUTabs() { return mMaxCPUDumpTabs; }
...
void CPUMultiDump::showDisassemblyTabSlot(duint selectionStart,
                                          duint selectionEnd,
                                          duint firstAddress) {
...
void CPUMultiDump::getDumpAttention() { mCurrentCPUDump->getAttention(); }

void CPUMultiDump::disassembleAtSlot(dsint addr, dsint pc) {
  if (!addr) addr = pc;
  mSourceView->setSelection(addr);
}

void CPUMultiDump::loadPseudoSourceSlot(const QString &src, duint base) {
  mSourceView->loadFile(src, base);
}
