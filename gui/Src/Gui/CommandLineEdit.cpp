...
int currentCommander = 0;

CommandLineEdit::CommandLineEdit(QWidget* parent)
    : HistoryLineEdit(parent), mCurrentScriptIndex(-1) {
  // QComboBox
  ...
  // Setup signals & slots
  connect(mCompleter, SIGNAL(activated(const QString&)), this, SLOT(clear()),
          Qt::QueuedConnection);
  connect(this, SIGNAL(textEdited(QString)), this,
          SLOT(autoCompleteUpdate(QString)));
  connect(Bridge::getBridge(), SIGNAL(autoCompleteAddCmd(QString)), this,
          SLOT(autoCompleteAddCmd(QString)));
  connect(Bridge::getBridge(), SIGNAL(autoCompleteDelCmd(QString)), this,
          SLOT(autoCompleteDelCmd(QString)));
  connect(Bridge::getBridge(), SIGNAL(autoCompleteClearAll()), this,
          SLOT(autoCompleteClearAll()));
  connect(Bridge::getBridge(), SIGNAL(registerScriptLang(SCRIPTTYPEINFO*)),
          this, SLOT(registerScriptType(SCRIPTTYPEINFO*)));
  connect(Bridge::getBridge(), SIGNAL(unregisterScriptLang(int)), this,
          SLOT(unregisterScriptType(int)));
  connect(mCmdScriptType, SIGNAL(currentIndexChanged(int)), this,
          SLOT(scriptTypeChanged(int)));
  connect(mCmdScriptType, SIGNAL(activated(int)), this,
          SLOT(scriptTypeActivated(int)));
  connect(Config(), SIGNAL(fontsUpdated()), this, SLOT(fontsUpdated()));

  fontsUpdated();
}

CommandLineEdit::~CommandLineEdit() { saveSettings("CommandLine"); }
    ...
    if (stringList.size()) {
      ...
      if (!popup->isVisible()) {
        for (int row = 0; row < popup->model()->rowCount(); row++) {
          ...
          // If the lineedit contains a suggested command, get the next
          // suggested one
          if (popup->model()->data(modelIndex) == this->text()) {
            int nextModelIndexRow =
                (currentModelIndex.row() + 1) % popup->model()->rowCount();
...
void CommandLineEdit::execute() {
  if (mCurrentScriptIndex == -1) return;
...
QWidget* CommandLineEdit::selectorWidget() { return mCmdScriptType; }
...
void CommandLineEdit::autoCompleteUpdate(const QString text) {
  ...
  if (text.length() <= 0) {
    ...
    GUISCRIPTCOMPLETER complete =
        mScriptInfo[mCurrentScriptIndex].completeCommand;
...
void CommandLineEdit::unregisterScriptType(int id) {
  ...
  if (mCurrentScriptIndex > 0) mCurrentScriptIndex--;
...
void CommandLineEdit::scriptTypeChanged(int index) {
  ...
  currentCommander = mCurrentScriptIndex;
