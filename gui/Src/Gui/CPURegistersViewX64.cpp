...
#define FLAGS EFLAGS
#define RegistersView RegistersViewX64
...
CPURegistersViewX64::CPURegistersViewX64(CPUWidget* parent)
...
void CPURegistersViewX64::refreshShortcutsSlot() {
...
void CPURegistersViewX64::mousePressEvent(QMouseEvent* event) {
  if (!isActive) return;
...
void CPURegistersViewX64::mouseDoubleClickEvent(QMouseEvent* event) {
...
void CPURegistersViewX64::keyPressEvent(QKeyEvent* event) {
  if (isActive) {
    int key = event->key();
...
void CPURegistersViewX64::debugStateChangedSlot(DBGSTATE state) {
  if (state == stopped) {
    updateRegistersSlot();
    isActive = false;
  } else {
...
void CPURegistersViewX64::updateRegistersSlot() {
  // read registers
...
void CPURegistersViewX64::ModifyFields(const QString& title,
                                       STRING_VALUE_TABLE_t* table,
                                       SIZE_T size) {
...
void CPURegistersViewX64::displayEditDialog() {
...
void CPURegistersViewX64::CreateDumpNMenu(QMenu* dumpMenu) {
  QList<QString> names;
...
void CPURegistersViewX64::onIncrementx87StackAction() {
#if 0
...
void CPURegistersViewX64::onDecrementx87StackAction() {
#if 0
    if(mFPUx87_80BITSDISPLAY.contains(mSelected))
        setRegister(x87SW_TOP, ((* ((duint*) registerValue(&wRegDumpStruct, x87SW_TOP))) - 1) % 8);
#endif
...
void CPURegistersViewX64::onIncrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) + 1);
}

void CPURegistersViewX64::onDecrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - 1);
}

void CPURegistersViewX64::onIncrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
...
void CPURegistersViewX64::onDecrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
        (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - sizeof(void*));
...
void CPURegistersViewX64::onPushAction() {
...
void CPURegistersViewX64::onPopAction() {
...
void CPURegistersViewX64::onZeroAction() {
...
void CPURegistersViewX64::onSetToOneAction() {
  if (mSETONEZEROTOGGLE.contains(mSelected)) {
    // if(mSelected >= x87r0 && mSelected <= x87r7 || mSelected >= x87st0 &&
    // mSelected <= x87st7)
    //    setRegister(mSelected,
...
void CPURegistersViewX64::onModifyAction() {
  if (mMODIFYDISPLAY.contains(mSelected)) displayEditDialog();
}

void CPURegistersViewX64::onToggleValueAction() {
...
void CPURegistersViewX64::onUndoAction() {
...
void CPURegistersViewX64::onCopyToClipboardAction() {
  Bridge::CopyToClipboard(GetRegStringValueFromValue(
      mSelected, registerValue(&wRegDumpStruct, mSelected)));
}

void CPURegistersViewX64::onCopyFloatingPointToClipboardAction() {
  Bridge::CopyToClipboard(ToLongDoubleString(
      ((X87FPUREGISTER*)registerValue(&wRegDumpStruct, mSelected))->data));
}

void CPURegistersViewX64::onCopySymbolToClipboardAction() {
...
void CPURegistersViewX64::onHighlightSlot() {
  Disassembly* CPUDisassemblyView = mParent->getDisasmWidget();
...
void CPURegistersViewX64::onFollowInDisassembly() {
...
void CPURegistersViewX64::onFollowInDump() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX64::onFollowInDumpN() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX64::onFollowInStack() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX64::onFollowInMemoryMap() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX64::displayCustomContextMenuSlot(QPoint pos) {
  if (!isActive) return;
  QMenu wMenu(this);
...
void CPURegistersViewX64::onSIMDMode() {
...
void CPURegistersViewX64::onFpuMode() {
  mFpuMode = (char)(dynamic_cast<QAction*>(sender())->data().toInt());
  InitMappings();
  emit refresh();
}

void CPURegistersViewX64::setRegister(REGISTER_NAME reg, duint value) {
...
void CPURegistersViewX64::disasmSelectionChangedSlot(dsint va) {
  mHighlightRegs =
