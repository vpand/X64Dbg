...
FavouriteTools::FavouriteTools(QWidget* parent)
    : QDialog(parent), ui(new Ui::FavouriteTools) {
  ui->setupUi(this);
  // set window flags
  ...
  for (i = 1; BridgeSettingGet("Favourite",
                               QString("Command%1").arg(i).toUtf8().constData(),
                               buffer);
       i++) {
  ...
  connect(ui->listTools, SIGNAL(itemSelectionChanged()), this,
          SLOT(onListSelectionChanged()));
  connect(ui->listScript, SIGNAL(itemSelectionChanged()), this,
          SLOT(onListSelectionChanged()));
  connect(ui->listCommand, SIGNAL(itemSelectionChanged()), this,
          SLOT(onListSelectionChanged()));
...
void FavouriteTools::setupTools(QString name, QTableWidget* list) {
  ...
  for (i = 1; BridgeSettingGet("Favourite",
                               (name + QString::number(i)).toUtf8().constData(),
                               buffer);
       i++) {
    ...
    if (BridgeSettingGet(
            "Favourite",
            (name + "Shortcut" + QString::number(i)).toUtf8().constData(),
            buffer))
      toolShortcut = QString(buffer);
    if (BridgeSettingGet(
            "Favourite",
            (name + "Description" + QString::number(i)).toUtf8().constData(),
            buffer))
...
void FavouriteTools::on_btnAddFavouriteTool_clicked() {
  ...
  BrowseDialog browse(
      this, tr("Browse tool"), tr("Enter the path of the tool."),
      tr("All Files (*.*)"), QString(buffer), false);
  if (browse.exec() != QDialog::Accepted || browse.path.length() == 0) return;
  ...
  if (rows == 0) ui->listTools->selectRow(0);
  updateToolsBtnEnabled();
}
...
void FavouriteTools::on_btnDescriptionFavouriteTool_clicked() {
  ...
  if (SimpleInputBox(this, tr("Enter the description"), description,
                     description, tr("This string will appear in the menu.")))
...
void FavouriteTools::on_btnAddFavouriteScript_clicked() {
  ...
  filename = QFileDialog::getOpenFileName(
      this, tr("Select script"), QString(buffer),
      tr("Script files (*.txt *.scr);;All files (*.*)"));
  if (filename.size() == 0) return;
  filename = QDir::toNativeSeparators(filename);
  BridgeSettingSet("Favourite", "LastScriptPath",
  ...
  if (rows == 0) ui->listScript->selectRow(0);
...
void FavouriteTools::on_btnEditFavouriteScript_clicked() {
  ...
  QString filename = table->item(table->currentRow(), 0)->text();
  filename = QFileDialog::getOpenFileName(
      this, tr("Select script"), filename,
      tr("Script files (*.txt *.scr);;All files (*.*)"));
  if (filename.size() == 0) return;
  filename = QDir::toNativeSeparators(filename);
  BridgeSettingSet("Favourite", "LastScriptPath",
...
void FavouriteTools::on_btnAddFavouriteCommand_clicked() {
  QString cmd;
  if (SimpleInputBox(this, tr("Enter the command you want to favourite"), "",
                     cmd, tr("Example: bphws csp"))) {
    ...
    if (rows == 0) ui->listCommand->selectRow(0);
...
void FavouriteTools::on_btnEditFavouriteCommand_clicked() {
  ...
  if (!table->rowCount()) return;
  QString cmd;
  if (SimpleInputBox(this, tr("Enter a new command"),
                     table->item(table->currentRow(), 0)->text(), cmd,
                     tr("Example: bphws ESP")) &&
      cmd.length())
...
void FavouriteTools::on_shortcutEdit_askForSave() {
  ...
  if (newKey != currentShortcut) {
    bool good = true;
    if (!newKey.isEmpty()) {
      for (auto i = Config()->Shortcuts.cbegin();
           i != Config()->Shortcuts.cend(); ++i) {
        if (i.value().Hotkey ==
...
void FavouriteTools::on_btnOK_clicked() {
  ...
  for (i = 1; i <= ui->listTools->rowCount(); i++) {
    BridgeSettingSet(
        ...
        ui->listTools->item(i - 1, 0)->text().toUtf8().constData());
    BridgeSettingSet(
        "Favourite", QString("ToolShortcut%1").arg(i).toUtf8().constData(),
        ui->listTools->item(i - 1, 1)->text().toUtf8().constData());
    BridgeSettingSet(
        "Favourite", QString("ToolDescription%1").arg(i).toUtf8().constData(),
        ui->listTools->item(i - 1, 2)->text().toUtf8().constData());
  }
  if (ui->listTools->rowCount() == 0) {
    ...
    {
      BridgeSettingSet("Favourite",
                       QString("Tool%1").arg(i).toUtf8().constData(), "");
      BridgeSettingSet("Favourite",
                       QString("ToolShortcut%1").arg(i).toUtf8().constData(),
                       "");
      BridgeSettingSet("Favourite",
                       QString("ToolDescription%1").arg(i).toUtf8().constData(),
                       "");
      i++;
  ...
  for (int i = 1; i <= ui->listScript->rowCount(); i++) {
    BridgeSettingSet(
        ...
        ui->listScript->item(i - 1, 0)->text().toUtf8().constData());
    BridgeSettingSet(
        "Favourite", QString("ScriptShortcut%1").arg(i).toUtf8().constData(),
        ui->listScript->item(i - 1, 1)->text().toUtf8().constData());
    BridgeSettingSet(
        "Favourite", QString("ScriptDescription%1").arg(i).toUtf8().constData(),
        ui->listScript->item(i - 1, 2)->text().toUtf8().constData());
  }
  if (ui->listScript->rowCount() == 0) {
    ...
    do {
      BridgeSettingSet("Favourite",
                       QString("Script%1").arg(i).toUtf8().constData(), "");
      BridgeSettingSet("Favourite",
                       QString("ScriptShortcut%1").arg(i).toUtf8().constData(),
                       "");
      BridgeSettingSet(
          "Favourite",
          QString("ScriptDescription%1").arg(i).toUtf8().constData(), "");
      i++;
  ...
  for (int i = 1; i <= ui->listCommand->rowCount(); i++) {
    BridgeSettingSet(
        ...
        ui->listCommand->item(i - 1, 0)->text().toUtf8().constData());
    BridgeSettingSet(
        "Favourite", QString("CommandShortcut%1").arg(i).toUtf8().constData(),
        ui->listCommand->item(i - 1, 1)->text().toUtf8().constData());
  }
  if (ui->listCommand->rowCount() == 0) {
    ...
    do {
      BridgeSettingSet("Favourite",
                       QString("Command%1").arg(i).toUtf8().constData(), "");
      BridgeSettingSet("Favourite",
                       QString("CommandShortcut%1").arg(i).toUtf8().constData(),
                       "");
      i++;
