...
int RegistersViewA32::getEstimateHeight() { return mRowsNeeded * mRowHeight; }

void RegistersViewA32::SetChangeButton(QPushButton* push_button) {
  mChangeViewButton = push_button;
  fontsUpdatedSlot();
}

void RegistersViewA32::InitMappings() {
  ...
  mRegisterRelativePlaces.insert(i, Register_Relative_Position(i - 1, i + 1));
  i++;
  mRegisterMapping.insert(i, "T");
  mRegisterPlaces.insert(i, Register_Position(offset++, 24, 3, 1));
  mRegisterRelativePlaces.insert(i, Register_Relative_Position(i - 1, i + 1));
  i++;
#endif
  ...
  if (mShowFpu) {
    ...
    if (mFpuMode == 1) {
      ...
      for (int j = 0; j < 32; j++) {
        ...
        temp = metrics.width(QApplication::translate("RegistersViewA32_ConstantsOfRegisters", "Nonzero")) / mCharWidth + 13;
        NextColumnPosition = std::max(NextColumnPosition, temp);
        temp = metrics.width(QApplication::translate("RegistersViewA32_ConstantsOfRegisters", "Zero")) / mCharWidth + 13;
        NextColumnPosition = std::max(NextColumnPosition, temp);
        temp = metrics.width(QApplication::translate("RegistersViewA32_ConstantsOfRegisters", "Special")) / mCharWidth + 13;
        NextColumnPosition = std::max(NextColumnPosition, temp);
        temp = metrics.width(QApplication::translate("RegistersViewA32_ConstantsOfRegisters", "Empty")) / mCharWidth + 13;
        NextColumnPosition = std::max(NextColumnPosition, temp);
...
RegistersViewA32::RegistersViewA32(QWidget* parent)
...
void RegistersViewA32::refreshShortcutsSlot() {}

/**
 * @brief RegistersViewA32::~RegistersViewA32 The destructor.
 */
RegistersViewA32::~RegistersViewA32() {}

void RegistersViewA32::fontsUpdatedSlot() {
  if (!isEnabled()) return;
...
void RegistersViewA32::displayCustomContextMenuSlot(QPoint pos) {}

void RegistersViewA32::ShowFPU(bool set_showfpu) {
 ...
 */
bool RegistersViewA32::identifyRegister(const int line, const int offset,
...
/**
 * @brief RegistersViewA32::helpRegister Get the help information of a register.
 ...
 */
QString RegistersViewA32::helpRegister(REGISTER_NAME reg) {
  switch (reg) {
...
void RegistersViewA32::mousePressEvent(QMouseEvent* event) {
  if (!isActive) return;
...
void RegistersViewA32::mouseMoveEvent(QMouseEvent* event) {
  if (!isActive) {
...
void RegistersViewA32::mouseDoubleClickEvent(QMouseEvent* event) {}

void RegistersViewA32::paintEvent(QPaintEvent* event) {
  Q_UNUSED(event);
...
void RegistersViewA32::keyPressEvent(QKeyEvent* event) {
  QScrollArea::keyPressEvent(event);
}

QSize RegistersViewA32::sizeHint() const {
  // 32 character width
  return QSize(32 * mCharWidth, this->viewport()->height());
}

void* RegistersViewA32::operator new(size_t size) {
  return _aligned_malloc(size, 16);
}

void RegistersViewA32::operator delete(void* p) { _aligned_free(p); }
 ...
 * @return                  the label
 */
QString RegistersViewA32::getRegisterLabel(REGISTER_NAME register_selected) {
...
/**
 * @brief RegistersViewA32::GetRegStringValueFromValue Get the textual
 ...
 * settings.
 */
QString RegistersViewA32::GetRegStringValueFromValue(REGISTER_NAME reg,
                                                     const char* value) {
  QString valueText;
...
STRING_VALUE_TABLE_t MxCsrRCValueStringTable[] = {
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Toward Zero"),
     MxCsr_RC_TOZERO},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters",
                       "Toward Positive"),
     MxCsr_RC_POSITIVE},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters",
                       "Toward Negative"),
     MxCsr_RC_NEGATIVE},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Round Near"),
     MxCsr_RC_NEAR}};
...
/*
unsigned int RegistersViewA32::GetMxCsrRCValueFromString(const char* string)
{
    int i;
...
*/
QString RegistersViewA32::GetMxCsrRCStateString(unsigned short state) {
  int i;
  ...
  for (i = 0;
    ...
    if (MxCsrRCValueStringTable[i].value == state)
      return QApplication::translate("RegistersViewA32_ConstantsOfRegisters",
...
static STRING_VALUE_TABLE_t ControlWordRCValueStringTable[] = {
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Truncate"),
     x87CW_RC_TRUNCATE},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Round Up"),
     x87CW_RC_UP},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Round Down"),
     x87CW_RC_DOWN},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Round Near"),
     x87CW_RC_NEAR}};
// WARNING: The following function is commented out because it is not used
...
// internationized environment.
/*
unsigned int RegistersViewA32::GetControlWordRCValueFromString(const char*
string)
{
    int i;
...
*/
QString RegistersViewA32::GetControlWordRCStateString(unsigned short state) {
  int i;
...
// internationized environment.
/*
unsigned int RegistersViewA32::GetStatusWordTOPValueFromString(const char*
string)
{
    int i;
...
*/
QString RegistersViewA32::GetStatusWordTOPStateString(unsigned short state) {
  int i;
...
static STRING_VALUE_TABLE_t ControlWordPCValueStringTable[] = {
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Real4"),
     x87CW_PC_REAL4},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Not Used"),
     x87CW_PC_NOTUSED},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Real8"),
     x87CW_PC_REAL8},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Real10"),
     x87CW_PC_REAL10}};
/*
//WARNING: The following function is commented out because it is not used
currently. If it is used again, it should be modified to keep working under
internationized environment. unsigned int
RegistersViewA32::GetControlWordPCValueFromString(const char* string)
{
    int i;
...
QString RegistersViewA32::GetControlWordPCStateString(unsigned short state) {
  int i;
...
static STRING_VALUE_TABLE_t TagWordValueStringTable[] = {
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Nonzero"),
     X87FPU_TAGWORD_NONZERO},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Zero"),
     X87FPU_TAGWORD_ZERO},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Special"),
     X87FPU_TAGWORD_SPECIAL},
    {QT_TRANSLATE_NOOP("RegistersViewA32_ConstantsOfRegisters", "Empty"),
     X87FPU_TAGWORD_EMPTY}};
// WARNING: The following function is commented out because it is not used
// currently. If it is used again, it should be modified to keep working under
// internationized environment.
/*
unsigned int RegistersViewA32::GetTagWordValueFromString(const char* string)
{
    int i;
...
*/
QString RegistersViewA32::GetTagWordStateString(unsigned short state) {
  int i;
...
void RegistersViewA32::drawRegister(QPainter* p, REGISTER_NAME reg,
                                    char* value) {
...
void RegistersViewA32::appendRegister(QString& text, REGISTER_NAME reg,
...
void RegistersViewA32::onChangeFPUViewAction() { ShowFPU(!mShowFpu); }

void RegistersViewA32::onCopyAllAction() {
  QString text;
#if 0
...
void RegistersViewA32::debugStateChangedSlot(DBGSTATE state) {}

void RegistersViewA32::reload() { this->viewport()->update(); }

SIZE_T RegistersViewA32::GetSizeRegister(const REGISTER_NAME reg_name) {
  SIZE_T size;
...
int RegistersViewA32::CompareRegisters(const REGISTER_NAME reg_name,
...
char* RegistersViewA32::registerValue(const REGDUMP* regdraw,
...
#else
  const REGDUMPA32* regd = (REGDUMPA32*)regdraw;
  switch (reg) {
    case FLAGS:
...
void RegistersViewA32::setRegisters(REGDUMP* regraw) {
  // tests if new-register-value == old-register-value holds
  REGDUMPA32* reg = (REGDUMPA32*)regraw;
...
void RegistersViewA32::ensureRegisterVisible(REGISTER_NAME reg) {
