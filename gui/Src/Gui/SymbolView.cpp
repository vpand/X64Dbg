...
class ModuleStdTable final : public StdTable {
 public:
  ...
  QColor getCellColor(int r, int c) override {
    ...
    if (c != ColModule && c != ColStatus) return mTextColor;
    switch (getStatus(r)) {
      default:
      case MODSYMUNLOADED:
  ...
  QString getCellContent(int r, int c) override {
    if (c != ColStatus) return StdTable::getCellContent(r, c);
    switch (getStatus(r)) {
      default:
      case MODSYMUNLOADED:
        return tr("Unloaded");
...
SymbolView::SymbolView(QWidget* parent)
    : QWidget(parent), ui(new Ui::SymbolView) {
  ui->setupUi(this);
  ...
  connect(Bridge::getBridge(), SIGNAL(clearSymbolLog()), this,
          SLOT(clearSymbolLogSlot()));
  connect(Bridge::getBridge(), SIGNAL(selectionSymmodGet(SELECTIONDATA*)), this,
  ...
  connect(mModuleList, SIGNAL(enterPressedSignal()), this,
          SLOT(moduleFollow()));
  connect(Bridge::getBridge(), SIGNAL(updateSymbolList(int, SYMBOLMODULEINFO*)),
          this, SLOT(updateSymbolList(int, SYMBOLMODULEINFO*)));
...
void SymbolView::setupContextMenu() {
...
#if 0
  mFollowSymbolImportAction =
      new QAction(DIcon("import.png"), tr("Follow &imported address"), this);
  connect(mFollowSymbolImportAction, SIGNAL(triggered(bool)), this,
          SLOT(symbolFollowImport()));
#endif
  ...
  mTraceModuleAction = new QAction(
      DIcon("trace.png"), tr("Hyper&Trace Module"), this);
  connect(mTraceModuleAction, SIGNAL(triggered()), this,
          SLOT(moduleHyperTrace()));

#if 0
  mFollowInMemMap = new QAction(DIcon("memmap_find_address_page.png"),
  ...
  connect(mFollowInMemMap, SIGNAL(triggered()), this,
          SLOT(moduleFollowMemMap()));

  mDownloadSymbolsAction = new QAction(
      DIcon("pdb.png"), tr("&Download Symbols for This Module"), this);
  ...
  connect(mDownloadSymbolsAction, SIGNAL(triggered()), this,
          SLOT(moduleDownloadSymbols()));

  mDownloadAllSymbolsAction = new QAction(
      DIcon("pdb.png"), tr("Download Symbols for &All Modules"), this);
  ...
  connect(mDownloadAllSymbolsAction, SIGNAL(triggered()), this,
          SLOT(moduleDownloadAllSymbols()));
#endif
  ...
  mBrowseInExplorer = new QAction(DIcon("browseinexplorer.png"),
                                  tr("Browse in Explorer"), this);
  ...
  mModSetUserAction =
      new QAction(DIcon("markasuser.png"), tr("Mark as &user module"), this);
  ...
  mModSetSystemAction = new QAction(DIcon("markassystem.png"),
                                    tr("Mark as &system module"), this);
  ...
  connect(mModSetSystemAction, SIGNAL(triggered()), this,
          SLOT(moduleSetSystem()));

  mModSetPartyAction =
      new QAction(DIcon("markasparty.png"), tr("Mark as &party..."), this);
  ...
  connect(mModSetPartyAction, SIGNAL(triggered()), this,
          SLOT(moduleSetParty()));

  mRefreshAction = new QAction(DIcon("hex.png"), tr("&Refresh"), this);
  this->addAction(mRefreshAction);
  mModuleList->addAction(mRefreshAction);
  connect(mRefreshAction, SIGNAL(triggered()), this, SLOT(refreshList()));
...
void SymbolView::clearSymbolLogSlot() { ui->symbolLogEdit->clear(); }
...
void SymbolView::moduleSelectionChanged(int index) {
  ...
  for (auto base : selectedModules) {
    DbgSymbolEnum(
        base,
        [](const SYMBOLPTR* info, void* userdata) {
          ((std::vector<SYMBOLPTR>*)userdata)->push_back(*info);
          return true;  // TODO: allow aborting (enumeration in a separate
                        // thread)
        },
        &data);
  }
...
void SymbolView::updateSymbolList(int module_count, SYMBOLMODULEINFO* modules) {
  ...
  for (int i = 0; i < module_count; i++) {
    ...
    switch (party) {
      ...
      case 1:
        ...
        break;
      default:
        mModuleList->stdList()->setCellContent(i, ColParty,
                                               tr("Party: %1").arg(party));
        break;
    }
  ...
  mModuleList->stdList()->reloadData();
  // NOTE: DO NOT CALL mModuleList->refreshSearchList() IT WILL DEGRADE
  // PERFORMANCE!
  if (modules) BridgeFree(modules);
}
...
void SymbolView::symbolDecompile() {
...
void SymbolView::moduleContextMenu(QMenu* wMenu) {
  ...
  wMenu->addSeparator();
  wMenu->addAction(mTraceModuleAction);
  wMenu->addSeparator();
  ...
  if (party != 0) wMenu->addAction(mModSetUserAction);
  if (party != 1) wMenu->addAction(mModSetSystemAction);
...
void SymbolView::moduleBrowse() {
  ...
  system(QString("open \"%1\"").arg(QFileInfo(szModPath).dir().absolutePath()).toUtf8().data());
#else
    QStringList arguments;
    arguments << QString("/select,");
    arguments << QString(szModPath);
    QProcess::startDetached(
        QString("%1/explorer.exe")
            .arg(QProcessEnvironment::systemEnvironment().value("windir")),
        arguments);
#endif
  }
}

void SymbolView::moduleHyperTrace() {
  DbgCmdExec(QString("modhypertrace %0")
                 .arg(mModuleList->mCurList->getCellContent(
...
void SymbolView::moduleLoad() {
  ...
  BrowseDialog browse(this, tr("Select DLL"),
                      tr("Enter the path of a DLL to load in the debuggee."),
                      tr("DLL Files (*.dll);;All Files (*.*)"), QString(),
                      false);
  if (browse.exec() != QDialog::Accepted && browse.path.length()) return;
...
void SymbolView::moduleFree() {
  ...
  QString moduleName = mModuleList->mCurList->getCellContent(
      mModuleList->mCurList->getInitialSelection(), ColModule);
  if (moduleName.length() != 0) {
    QMessageBox::StandardButton reply;
    QString question =
        tr("Are you sure you want to free the module: %1?\n\nThis could "
           "introduce unexpected behaviour to your debugging session...")
            .arg(moduleName);
    reply = QMessageBox::question(this, tr("Free Library").toUtf8().constData(),
...
void SymbolView::moduleSetSystem() {
  ...
  duint modbase = DbgValFromString(
      mModuleList->mCurList->getCellContent(i, ColBase).toUtf8().constData());
...
void SymbolView::moduleSetParty() {
  ...
  if (SimpleInputBox(this, tr("Mark the party of the module as"),
                     QString::number(party), mLineEditeditText,
                     tr("0 is user module, 1 is system module."), &bookmark)) {
    bool ok;
    ...
    if (ok) {
      ...
      case 1:
          ...
          break;
      default:
          mModuleList->mCurList->setCellContent(i, 2, tr("Party:
      %1").arg(party)); break;
      }
      mModuleList->mCurList->reloadData();*/
    } else
      SimpleErrorBox(this, tr("Error"),
                     tr("The party number can only be 0 or 1"));
  }
}

void SymbolView::moduleFollowMemMap() {
  QString base = mModuleList->mCurList->getCellContent(
      mModuleList->mCurList->getInitialSelection(), ColBase);
...
void SymbolView::refreshList() { DbgCmdExec("refresh module"); }

void SymbolView::funcSelectionChanged(int index) {
  if (!DbgIsDebugging()) return;

  ZehSymbolTable* table = (ZehSymbolTable *)(
      sender() == mSymbolSearchList->list() ? mSymbolSearchList->list() : mSymbolSearchList->searchList());
  if (index >= table->mData.size()) return;

  ManaDatabase *mdb = (ManaDatabase *)table->mData[index].modbase;
  QString straddr = table->getCellContent(index, 0);
  duint addr = strtoul(straddr.toUtf8().data(), nullptr, 16);
  ...
  if (func) {
    ...
    if (sect) {
      addMsgToSymbolLogSlot(QString("%1.%2 %3-%4 %5\n")
               .arg(mdb->db->string(sect->info.name))
