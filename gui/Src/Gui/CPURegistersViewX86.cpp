...
#define FLAGS EFLAGS
#define RegistersView RegistersViewX86
...
CPURegistersViewX86::CPURegistersViewX86(CPUWidget* parent)
...
void CPURegistersViewX86::refreshShortcutsSlot() {
...
void CPURegistersViewX86::mousePressEvent(QMouseEvent* event) {
  if (!isActive) return;
...
void CPURegistersViewX86::mouseDoubleClickEvent(QMouseEvent* event) {
...
void CPURegistersViewX86::keyPressEvent(QKeyEvent* event) {
  if (isActive) {
    int key = event->key();
...
void CPURegistersViewX86::debugStateChangedSlot(DBGSTATE state) {
  if (state == stopped) {
    updateRegistersSlot();
    isActive = false;
  } else {
...
void CPURegistersViewX86::updateRegistersSlot() {
  // read registers
...
void CPURegistersViewX86::ModifyFields(const QString& title,
...
void CPURegistersViewX86::displayEditDialog() {
...
void CPURegistersViewX86::CreateDumpNMenu(QMenu* dumpMenu) {
  QList<QString> names;
...
void CPURegistersViewX86::onIncrementx87StackAction() {
#if 0
...
void CPURegistersViewX86::onDecrementx87StackAction() {
#if 0
    if(mFPUx87_80BITSDISPLAY.contains(mSelected))
        setRegister(x87SW_TOP, ((* ((duint*) registerValue(&wRegDumpStruct, x87SW_TOP))) - 1) % 8);
#endif
...
void CPURegistersViewX86::onIncrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) + 1);
}

void CPURegistersViewX86::onDecrementAction() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(mSelected,
                (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - 1);
}

void CPURegistersViewX86::onIncrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
...
void CPURegistersViewX86::onDecrementPtrSize() {
  if (mINCREMENTDECREMET.contains(mSelected))
    setRegister(
        mSelected,
        (*((duint*)registerValue(&wRegDumpStruct, mSelected))) - sizeof(void*));
...
void CPURegistersViewX86::onPushAction() {
...
void CPURegistersViewX86::onPopAction() {
...
void CPURegistersViewX86::onZeroAction() {
...
void CPURegistersViewX86::onSetToOneAction() {
  if (mSETONEZEROTOGGLE.contains(mSelected)) {
    // if(mSelected >= x87r0 && mSelected <= x87r7 || mSelected >= x87st0 &&
    // mSelected <= x87st7)
    //    setRegister(mSelected,
...
void CPURegistersViewX86::onModifyAction() {
  if (mMODIFYDISPLAY.contains(mSelected)) displayEditDialog();
}

void CPURegistersViewX86::onToggleValueAction() {
...
void CPURegistersViewX86::onUndoAction() {
...
void CPURegistersViewX86::onCopyToClipboardAction() {
  Bridge::CopyToClipboard(GetRegStringValueFromValue(
      mSelected, registerValue(&wRegDumpStruct, mSelected)));
}

void CPURegistersViewX86::onCopyFloatingPointToClipboardAction() {
  Bridge::CopyToClipboard(ToLongDoubleString(
      ((X87FPUREGISTER*)registerValue(&wRegDumpStruct, mSelected))->data));
}

void CPURegistersViewX86::onCopySymbolToClipboardAction() {
...
void CPURegistersViewX86::onHighlightSlot() {
  Disassembly* CPUDisassemblyView = mParent->getDisasmWidget();
...
void CPURegistersViewX86::onFollowInDisassembly() {
...
void CPURegistersViewX86::onFollowInDump() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX86::onFollowInDumpN() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX86::onFollowInStack() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX86::onFollowInMemoryMap() {
  if (mCANSTOREADDRESS.contains(mSelected)) {
    QString addr =
        QString("%1")
            .arg((*((duint*)registerValue(&wRegDumpStruct, mSelected))),
...
void CPURegistersViewX86::displayCustomContextMenuSlot(QPoint pos) {
  if (!isActive) return;
  QMenu wMenu(this);
...
void CPURegistersViewX86::onSIMDMode() {
...
void CPURegistersViewX86::onFpuMode() {
  mFpuMode = (char)(dynamic_cast<QAction*>(sender())->data().toInt());
  InitMappings();
  emit refresh();
}

void CPURegistersViewX86::setRegister(REGISTER_NAME reg, duint value) {
...
void CPURegistersViewX86::disasmSelectionChangedSlot(dsint va) {
  mHighlightRegs =
