...
void AbstractTableView::closeSlot() {
  if (ConfigBool("Gui", "SaveColumnOrder")) saveColumnToConfig();
}
...
void AbstractTableView::updateColorsSlot() { updateColors(); }

void AbstractTableView::updateFontsSlot() { updateFonts(); }

void AbstractTableView::updateShortcutsSlot() { updateShortcuts(); }

void AbstractTableView::loadColumnFromConfig(const QString& viewName) {
  int columnCount = getColumnCount();
  for (int i = 0; i < columnCount; i++) {
    duint width = ConfigUint(
        "Gui",
        QString("%1ColumnWidth%2").arg(viewName).arg(i).toUtf8().constData());
    duint hidden = ConfigUint(
        "Gui",
        QString("%1ColumnHidden%2").arg(viewName).arg(i).toUtf8().constData());
    duint order = ConfigUint(
        "Gui",
        QString("%1ColumnOrder%2").arg(viewName).arg(i).toUtf8().constData());
    if (width != 0) setColumnWidth(i, width);
    if (hidden != 2) setColumnHidden(i, !!hidden);
    if (order != 0) mColumnOrder[i] = order - 1;
  }
  mViewName = viewName;
}

void AbstractTableView::saveColumnToConfig() {
  if (mViewName.length() == 0) return;
  int columnCount = getColumnCount();
  auto cfg = Config();
  for (int i = 0; i < columnCount; i++) {
    cfg->setUint(
        "Gui",
        QString("%1ColumnWidth%2").arg(mViewName).arg(i).toUtf8().constData(),
        getColumnWidth(i));
    cfg->setUint(
        "Gui",
        QString("%1ColumnHidden%2").arg(mViewName).arg(i).toUtf8().constData(),
        getColumnHidden(i) ? 1 : 0);
    cfg->setUint(
        "Gui",
        QString("%1ColumnOrder%2").arg(mViewName).arg(i).toUtf8().constData(),
        mColumnOrder[i] + 1);
  }
}

void AbstractTableView::setupColumnConfigDefaultValue(QMap<QString, duint>& map,
...
void AbstractTableView::paintEvent(QPaintEvent* event) {
  if (!mAllowPainting || GuiIsUpdateDisabled()) return;

  if (getColumnCount())  // make sure the last column is never smaller than the
                         // window
  {
    int totalWidth = 0;
    ...
    for (int i = 0; i < getColumnCount(); i++) {
      if (getColumnHidden(mColumnOrder[i])) continue;
      last = mColumnOrder[i];
  ...
  if (mHeader.isVisible == true) {
    for (int j = 0; j < getColumnCount(); j++) {
      ...
      if (getColumnHidden(i)) continue;
      int width = getColumnWidth(i);
      QStyleOptionButton wOpt;
      if ((mColumnList[i].header.isPressed == true) &&
              (mColumnList[i].header.isMouseOver == true) ||
          (mGuiState == AbstractTableView::HeaderButtonReordering &&
           mColumnOrder[mHoveredColumnDisplayIndex] == i))
      ...
      mHeaderButtonSytle.style()->drawControl(QStyle::CE_PushButton, &wOpt,
                                              &wPainter, &mHeaderButtonSytle);

      wPainter.setPen(mHeaderTextColor);
      wPainter.drawText(QRect(x + 4, y, width - 8, getHeaderHeight()),
                        Qt::AlignVCenter | Qt::AlignLeft, mColumnList[i].title);
    ...
    for (int i = 0; i < wViewableRowsCount; i++) {
      ...
      if (i < mNbrOfLineToPrint) {
        ...
        if (!mDrawDebugOnly || DbgIsDebugging()) {
          wStr = paintContent(&wPainter, mTableOffset, i, j, x, y,
                              getColumnWidth(j), getRowHeight());

          if (wStr.length()) {
            wPainter.setPen(getCellColor(mTableOffset + i, j));
            wPainter.drawText(
                QRect(x + 4, y, getColumnWidth(j) - 4, getRowHeight()),
                Qt::AlignVCenter | Qt::AlignLeft, wStr);
          }
        }
      }
      ...
      if (getColumnCount() > 1) {
        ...
        wPainter.drawLine(x + getColumnWidth(j) - 1, y,
                          x + getColumnWidth(j) - 1, y + getRowHeight() - 1);
      }
...
void AbstractTableView::mouseMoveEvent(QMouseEvent* event) {
  ...
  int wStartPos =
      getColumnPosition(wDisplayIndex);  // Position X of the start of column
  int wEndPos =
      wStartPos + getColumnWidth(wColIndex);  // Position X of the end of column
  bool wHandle = ((wColIndex != 0) && (event->x() >= wStartPos) &&
                  (event->x() <= (wStartPos + 2))) ||
                 ((event->x() <= wEndPos) && (event->x() >= (wEndPos - 2)));
  if (wColIndex == getColumnCount() - 1 &&
      event->x() > viewport()->width())  // last column
    wHandle = false;
  ...
  switch (mGuiState) {
        ...
        if ((wHandle == true) && (wHasCursor == false)) {
          ...
          if (event->y() > getHeaderHeight() && DbgIsDebugging())
            ShowDisassemblyPopup(
                getDisassemblyPopupAddress(event->x(), event->y()), event->x(),
                event->y());
        }
      } else {
    ...
    case AbstractTableView::ResizeColumnState: {
      ...
      bool bCanResize =
          (getColumnWidth(mColumnOrder[mColResizeData.index]) + delta) >= 20;
      if (bCanResize) {
        int wNewSize =
    ...
    case AbstractTableView::HeaderButtonPressed: {
      ...
      if (wColIndex == mHeader.activeButtonIndex) {
        mColumnList[mHeader.activeButtonIndex].header.isMouseOver =
            (event->y() <= getHeaderHeight()) && (event->y() >= 0);
        break;
      } else {
        mGuiState = AbstractTableView::HeaderButtonReordering;
      }
...
void AbstractTableView::mousePressEvent(QMouseEvent* event) {
  if (((event->buttons() & Qt::LeftButton) != 0) &&
    ...
    if (mColResizeData.splitHandle == true) {
      ...
      int wDisplayIndex = getColumnDisplayIndexFromX(event->x());
      int wStartPos = getColumnPosition(
          wDisplayIndex);  // Position X of the start of column
      ...
      mColResizeData.lastPosX = event->x();
    } else if (mHeader.isVisible && getColumnCount() &&
               (event->y() <= getHeaderHeight()) && (event->y() >= 0)) {
  ...
  if ((event->buttons() & Qt::LeftButton) == 0) {
    if (mGuiState == AbstractTableView::ResizeColumnState) {
      ...
      if (mColumnList[mHeader.activeButtonIndex].header.isMouseOver == true) {
        // qDebug() << "Button " << mHeader.activeButtonIndex << "has been
        // released.";
...
void AbstractTableView::wheelEvent(QWheelEvent* event) {
  ...
  if (!numDegrees) {
    if (up_count > 15 || down_count > 15) {
      if (up_count > down_count) numDegrees = 15;
      else numDegrees = -15;
    }
    else {
      if (event->delta() > 0) up_count++;
      else down_count++;
      return;
    }
  }
...
/**
 * @brief       This method has been reimplemented. It repaints the table when
 * the height changes.
 *
...
void AbstractTableView::keyPressEvent(QKeyEvent* event) {
  ...
  if (wKey == Qt::Key_Up) {
    ...
    verticalScrollBar()->triggerAction(QAbstractSlider::SliderPageStepAdd);
  } else if (wKey == Qt::Key_Return ||
             wKey == Qt::Key_Enter)  // user pressed enter
 ...
 ***********************************************************************************/
/**
 * @brief       This method is the slot connected to the actionTriggered signal
...
void AbstractTableView::vertSliderActionSlot(int action) {
  ...
  // Bounding
  wSliderPos = wSliderPos > verticalScrollBar()->maximum()
...
#ifdef _WIN64
      wDelta = scaleFromScrollBarRangeToUint64(wSliderPos) -
               (mTableOffset - mTableOffsetBase);
#else
...
/**
 * @brief       This virtual method is called at the end of the
 * vertSliderActionSlot(...) method. It allows changing the table offset
 * according to the action type, the old table offset and delta between the old
 * and the new table offset.
 *
 * @param[in]   type      Type of action (Refer to the
 * QAbstractSlider::SliderAction enum)
...
/**
 * @brief       This method scale the given 64bits integer to the scrollbar
 * range (32bits).
 *
...
int AbstractTableView::scaleFromUint64ToScrollBarRange(dsint value) {
  if (mScrollBarAttributes.is64 == true) {
    ...
    dsint wValueMax =
        ((dsint)getRowCount() - 1) >> mScrollBarAttributes.rightShiftCount;

    if (value == ((dsint)getRowCount() - 1))
      return (int)(verticalScrollBar()->maximum());
    else
      return (
          int)((dsint)((dsint)verticalScrollBar()->maximum() * (dsint)wValue) /
               (dsint)wValueMax);
  } else {
    return (int)(value - mTableOffsetBase);
  }
}
#endif

/**
 * @brief       This method scale the given 32bits integer to the table range
 * (64bits).
 *
...
dsint AbstractTableView::scaleFromScrollBarRangeToUint64(int value) {
  if (mScrollBarAttributes.is64 == true) {
    ...
    if (value == (int)0x7FFFFFFF)
      ...
      return (dsint)(
          ((dsint)((dsint)wValueMax * (dsint)value) / (dsint)0x7FFFFFFF)
...
/**
 * @brief       This method updates the vertical scrollbar range and
 * pre-computes some attributes for the 32<->64bits conversion methods.
 *
...
************************************************************************************/
/**
 * @brief       Returns the index offset (relative to the table offset)
 * corresponding to the given y coordinate.
 *
 * @param[in]   y      Pixel offset starting from the top of the table (without
 * the header)
 *
...
/**
 * @brief       Returns the index of the column corresponding to the given x
 * coordinate.
 *
...
/**
 * @brief       Returns the displayed index of the column corresponding to the
 * given x coordinate.
 *
 * @param[in]   x      Pixel offset starting from the left of the table
 *
...
/**
 * @brief       Returns the x coordinate of the beginning of the column at index
 * index.
 *
 ...
 */
int AbstractTableView::transY(int y) const { return y - getHeaderHeight(); }

/**
 * @brief       Returns the number of viewable rows in the current window
 * (Partially viewable rows are aslo counted).
 *
...
/**
 * @brief       This virtual method returns the number of remaining lines to
 * print.
 *
...
int AbstractTableView::getLineToPrintcount() const {
  ...
  int wCount = (dsint)wRemainingRowsCount > (dsint)wViewableRowsCount
                   ? (int)wViewableRowsCount
                   : (int)wRemainingRowsCount;
  return wCount;
}
 ...
 * @param[in]   isClickable     Boolean that tells whether the header is
 * clickable or not
 * @return      Nothing.
 */
void AbstractTableView::addColumnAt(int width, const QString& title,
                                    bool isClickable) {
...
void AbstractTableView::setRowCount(dsint count) {
  if (mRowCount != count) mShouldReload = true;
  mRowCount = count;
}
...
/************************************************************************************
                                Getter & Setter
************************************************************************************/
dsint AbstractTableView::getRowCount() const { return mRowCount; }

int AbstractTableView::getColumnCount() const { return mColumnList.size(); }
...
void AbstractTableView::setColumnHidden(int col, bool hidden) {
  if (col < getColumnCount() && col >= 0) mColumnList[col].hidden = hidden;
}
...
void AbstractTableView::setColumnWidth(int index, int width) {
  ...
  MethodInvoker::invokeMethod([this]() {
    int totalWidth = 0;
    for (int i = 0; i < getColumnCount(); i++)
      if (!getColumnHidden(i)) totalWidth += getColumnWidth(i);
...
void AbstractTableView::setColumnOrder(int pos, int index) {
  if (index != 0) mColumnOrder[pos] = index - 1;
}
...
int AbstractTableView::getGuiState() const { return mGuiState; }

int AbstractTableView::getNbrOfLineToPrint() const { return mNbrOfLineToPrint; }

void AbstractTableView::setNbrOfLineToPrint(int parNbrOfLineToPrint) {
  mNbrOfLineToPrint = parNbrOfLineToPrint;
}

void AbstractTableView::setShowHeader(bool show) { mHeader.isVisible = show; }
...
bool AbstractTableView::getDrawDebugOnly() const { return mDrawDebugOnly; }

void AbstractTableView::setDrawDebugOnly(bool value) { mDrawDebugOnly = value; }

void AbstractTableView::setAllowPainting(bool allow) { mAllowPainting = allow; }

bool AbstractTableView::getAllowPainting() const { return mAllowPainting; }

/************************************************************************************
                           Table offset management
************************************************************************************/
dsint AbstractTableView::getTableOffset() const { return mTableOffset; }
...
void AbstractTableView::setTableOffset(dsint val) {
  ...
  if (val > wMaxOffset) return;

  // If val is within the last ViewableRows, then set RVA to rowCount -
  // ViewableRows + 1
...
void AbstractTableView::updateViewport() {
  MethodInvoker::invokeMethod([this]() { viewport()->update(); });
}

/**
 * @brief       This method is called when data have to be reloaded (e.g. When
 * table offset changes).
 *
 * @return      Nothing.
 */
void AbstractTableView::prepareData() {
  int wViewableRowsCount = getViewableRowsCount();
  dsint wRemainingRowsCount = getRowCount() - mTableOffset;
  mNbrOfLineToPrint = (dsint)wRemainingRowsCount > (dsint)wViewableRowsCount
                          ? (int)wViewableRowsCount
                          : (int)wRemainingRowsCount;
}
...
void AbstractTableView::ShowDisassemblyPopup(duint addr, int x, int y) {
  if (!mPopupEnabled || !addr) {
    ...
    mPopupTimer = 0;
    if (mDisassemblyPopup) mDisassemblyPopup->hide();
    return;
  }
  if (!mDisassemblyPopup) mDisassemblyPopup = new DisassemblyPopup(this);
  if (mDisassemblyPopup->getAddress() == addr) return;
  if (DbgFunctions()->MemIsCodePage(addr, false)) {
    mDisassemblyPopup->move(
        mapToGlobal(QPoint(x + 20, y + fontMetrics().height() * 2)));
