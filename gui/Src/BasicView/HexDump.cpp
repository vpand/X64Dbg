...
HexDump::HexDump(QWidget* parent) : AbstractTableView(parent) {
  ...
  connect(Bridge::getBridge(), SIGNAL(dbgStateChanged(DBGSTATE)), this,
          SLOT(debugStateChanged(DBGSTATE)));
  setupCopyMenu();

  Initialize();
}

HexDump::~HexDump() { delete mMemPage; }
...
void HexDump::copySelectionSlot() { Bridge::CopyToClipboard(makeCopyText()); }
...
void HexDump::printDumpAt(dsint parVA, bool select, bool repaint,
  ...
  mMemPage->setAttributes(
      wBase, wSize);  // Set base and size (Useful when memory page changed)
...
void HexDump::printDumpAt(dsint parVA) { printDumpAt(parVA, true); }

void HexDump::gotoPreviousSlot() { printDumpAt(mHistory.historyPrev()); }

void HexDump::gotoNextSlot() { printDumpAt(mHistory.historyNext()); }

duint HexDump::rvaToVa(dsint rva) const { return mMemPage->va(rva); }
...
QString HexDump::paintContent(QPainter* painter, dsint rowBase, int rowOffset,
...
void HexDump::printSelected(QPainter* painter, dsint rowBase, int rowOffset,
                            int col, int x, int y, int w, int h) {
...
dsint HexDump::getSelectionStart() const { return mSelection.fromIndex; }

dsint HexDump::getSelectionEnd() const { return mSelection.toIndex; }

bool HexDump::isSelected(dsint rva) const {
  return rva >= mSelection.fromIndex && rva <= mSelection.toIndex;
}

void HexDump::getColumnRichText(int col, dsint rva,
                                RichTextPainter::List& richText) {
...
void HexDump::byteToString(duint rva, byte_t byte, ByteViewMode mode,
...
void HexDump::wordToString(duint rva, uint16 word, WordViewMode mode,
                           RichTextPainter::CustomRichText_t& richText) {
  Q_UNUSED(rva);
  QString wStr;
...
void HexDump::dwordToString(duint rva, uint32 dword, DwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText) {
  Q_UNUSED(rva);
  QString wStr;
...
void HexDump::qwordToString(duint rva, uint64 qword, QwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText) {
  Q_UNUSED(rva);
  QString wStr;
...
void HexDump::twordToString(duint rva, void* tword, TwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText) {
  Q_UNUSED(rva);
  QString wStr;
...
int HexDump::getSizeOf(DataSize size) { return int(size); }
...
void HexDump::appendDescriptor(int width, QString title, bool clickable,
...
void HexDump::appendResetDescriptor(int width, QString title, bool clickable,
                                    ColumnDescriptor descriptor) {
