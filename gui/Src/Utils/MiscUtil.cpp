...
bool SimpleInputBox(QWidget* parent, const QString& title, QString defaultValue,
                    QString& output, const QString& placeholderText,
                    const QIcon* icon) {
...
bool SimpleChoiceBox(QWidget* parent, const QString& title,
                     QString defaultValue, const QStringList& choices,
                     QString& output, bool editable,
                     const QString& placeholderText, const QIcon* icon,
...
void SimpleErrorBox(QWidget* parent, const QString& title,
                    const QString& text) {
  QMessageBox msg(QMessageBox::Critical, title, text, QMessageBox::NoButton,
                  parent);
...
void SimpleWarningBox(QWidget* parent, const QString& title,
                      const QString& text) {
  QMessageBox msg(QMessageBox::Warning, title, text, QMessageBox::NoButton,
                  parent);
...
void SimpleInfoBox(QWidget* parent, const QString& title, const QString& text) {
  QMessageBox msg(QMessageBox::Information, title, text, QMessageBox::NoButton,
                  parent);
...
QString getSymbolicName(duint addr) {
  ...
  bool bHasModule = (DbgGetModuleAt(addr, moduleText) &&
...
QString getSymbolicNameStr(duint addr) {
  ...
  bool bHasModule = (DbgGetModuleAt(addr, moduleText) &&
                     !QString(labelText).startsWith("JMP.&"));
  QString addrText =
      DbgMemIsValidReadPtr(addr) ? ToPtrString(addr) : ToHexString(addr);
  QString finalText;
  if (bHasString)
...
QIcon getFileIcon(QString file) {
  SHFILEINFO info;
  if (SHGetFileInfoW((const wchar_t*)file.utf16(), 0, &info, sizeof(info),
                     SHGFI_ICON) == 0)
...
bool ExportCSV(dsint rows, dsint columns, std::vector<QString> headers,
               std::function<QString(dsint, dsint)> getCellContent) {
  BrowseDialog browse(
      nullptr,
      ...
      QApplication::translate("ExportCSV",
                              "CSV files (*.csv);;All files (*.*)"),
      QApplication::applicationDirPath() + QDir::separator() + "db", true);
  browse.setWindowIcon(DIcon("database-export.png"));
  if (browse.exec() == QDialog::Accepted) {
    FILE* csv;
    bool utf16;
    csv = fopen_helper(browse.path.toStdString().c_str(), "wb");
    if (csv == NULL) {
      GuiAddLogMessage(
      ...
      if (headers.size() > 0) {
        for (column = 0; column < columns; column++) {
          ...
          if (cell.contains('"') || cell.contains(',') || cell.contains('\r') ||
              cell.contains('\n')) {
            if (cell.contains('"')) cell = cell.replace("\"", "\"\"");
            cell = "\"" + cell + "\"";
          }
          if (column != columns - 1) cell = cell + ",";
          text = text + cell;
        }
        if (utf16) {
          text = text + "\r\n";
...
FILE *fopen_helper(const char *path, const char *mode) {
#if __APPLE__ || __linux__
  return fopen(path, mode);
#else
  return _wfopen(QString(path).toStdWString().data(), QString(mode).toStdWString().data());
#endif
}
