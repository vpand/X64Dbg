...
bool SymbolFromAddressExact(duint address, SymbolInfo& symInfo) {
  ...
  if (modInfo) {
    ...
    if (modInfo->symbols->isOpen()) {
      if (modInfo->symbols->findSymbolExact(rva, symInfo)) return true;
    }
...
bool SymbolFromAddressExactOrLower(duint address, SymbolInfo& symInfo) {
  ...
  MODINFO* modInfo = ModInfoFromAddr(address);
  if (modInfo) {
    if (modInfo->symbols->isOpen() == false) return false;
...
bool SymbolAdd(const SymbolInfo & symbol)
{
    ...
    foundR->second.insert({ Range(symbol.addr, symbol.addr + symbol.size - dec),
symbol }); auto hash = ModHashFromName(symbol.decoratedName.c_str());
