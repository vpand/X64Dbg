...
bool pluginload(const char* pluginName, bool loadall) {
...
#ifdef _WIN64
  sprintf_s(searchName, "%s\\%s",
            StringUtils::Utf16ToUtf8(pluginDirectory.c_str()).c_str(), name);
#else
  sprintf_s(searchName, "%s\\%s",
            StringUtils::Utf16ToUtf8(pluginDirectory.c_str()).c_str(), name);
#endif  // _WIN64
  ...
  if (!loadall &&
      !PathFileExistsW(StringUtils::Utf8ToUtf16(searchName).c_str())) {
  ...
  pluginData.hPlugin = LoadLibraryW(
      StringUtils::Utf8ToUtf16(searchName).c_str());  // load the plugin library
  ...
  pluginData.pluginit =
      (PLUGINIT)GetProcAddress(pluginData.hPlugin, "pluginit");
  if (!pluginData.pluginit) {
    dprintf(
        QT_TRANSLATE_NOOP(
            "DBG", "[PLUGIN] Export \"pluginit\" not found in plugin: %s\n"),
        name);
  ...
  pluginData.plugstop =
      (PLUGSTOP)GetProcAddress(pluginData.hPlugin, "plugstop");
  pluginData.plugsetup =
      (PLUGSETUP)GetProcAddress(pluginData.hPlugin, "plugsetup");
  ...
  if (!pluginData.pluginit(&pluginData.initStruct)) {
    dprintf(
        QT_TRANSLATE_NOOP("DBG", "[PLUGIN] pluginit failed for plugin: %s\n"),
        name);
    for (int i = CB_INITDEBUG; i < CB_LAST; i++)
      pluginunregistercallback(curPluginHandle, CBTYPE(i));
    FreeLibrary(pluginData.hPlugin);
  ...
  if (pluginData.initStruct.sdkVersion <
      PLUG_SDKVERSION)  // the plugin SDK is not compatible
  {
    dprintf(QT_TRANSLATE_NOOP(
                "DBG", "[PLUGIN] %s is incompatible with this SDK version\n"),
  ...
  dprintf(QT_TRANSLATE_NOOP("DBG", "[PLUGIN] %s v%d Loaded!\n"),
          pluginData.initStruct.pluginName,
          pluginData.initStruct.pluginVersion);
  ...
  auto regExport = [](const char* exportname, CBTYPE cbType) {
    ...
    if (cbPlugin) pluginregistercallback(curPluginHandle, cbType, cbPlugin);
  };
  ...
  // add plugin menus
  {
    ...
    auto addPluginMenu = [](GUIMENUTYPE type) {
      ...
      if (hNewMenu == -1) {
        dprintf(QT_TRANSLATE_NOOP(
                    "DBG", "[PLUGIN] GuiMenuAdd(%d) failed for plugin: %s\n"),
  ...
  if (!loadall) SetCurrentDirectoryW(currentDir);
  return true;
}
...
bool pluginunload(const char* pluginName, bool unloadall) {
  ...
  {
    EXCLUSIVE_ACQUIRE(LockPluginList);
    found = std::find_if(pluginList.begin(), pluginList.end(),
                         [&name](const PLUG_DATA& a) {
  ...
  if (found != pluginList.end()) {
    ...
    if (currentPlugin.plugstop) canFreeLibrary = currentPlugin.plugstop();
    ...
    }
    {
      ...
      if (!unloadall) {
        // remove from main pluginlist. We do this so unloadall doesn't try to
        // unload an already released plugin
        pluginList.erase(found);
      }
    }

    if (canFreeLibrary) FreeLibrary(currentPlugin.hPlugin);
...
void pluginloadall(const char* pluginDir) {
  ...
  HANDLE hSearch =
      FindFirstFileW(StringUtils::Utf8ToUtf16(searchName).c_str(), &foundData);
...
void pluginunloadall() {
  EXCLUSIVE_ACQUIRE(LockPluginList);
  for (const auto& plugin : pluginList) pluginunload(plugin.plugname, true);
  pluginList.clear();
}
...
\param pluginHandle Handle of the plugin to remove the expression functions
from.
*/
...
void pluginformatfuncunregisterall(int pluginHandle) {
  ...
  auto formatFuncList =
      pluginFormatfunctionList;  // copy for thread-safety reasons
  SHARED_RELEASE();
...
*/
void pluginregistercallback(int pluginHandle, CBTYPE cbType,
                            CBPLUGIN cbPlugin) {
...
\param [in,out] callbackInfo Information describing the callback. See plugin
documentation for more information on this.
*/
void plugincbcall(CBTYPE cbType, void* callbackInfo) {
  if (pluginCallbackList[cbType].empty()) return;
...
*/
bool plugincbempty(CBTYPE cbType) { return pluginCallbackList[cbType].empty(); }
...
\param command The command text to register. This text cannot contain the '\1'
character. This text is not case sensitive. \param cbCommand The command
callback. \param debugonly true if the command can only be called during
debugging. \return true if it the registration succeeded, false otherwise.
*/
bool plugincmdregister(int pluginHandle, const char* command,
                       CBPLUGINCOMMAND cbCommand, bool debugonly) {
  if (!command || strlen(command) >= deflen || strstr(command, "\1"))
    return false;
  String plugName;
  if (!findPluginName(pluginHandle, plugName)) return false;
  ...
  if (!dbgcmdnew(command, (CBCOMMAND)cbCommand, debugonly)) {
    dprintf(QT_TRANSLATE_NOOP(
                "DBG", "[PLUGIN, %s] Command \"%s\" failed to register...\n"),
...
bool plugincmdunregister(int pluginHandle, const char* command) {
  ...
  if (!findPluginName(pluginHandle, plugName)) return false;
  EXCLUSIVE_ACQUIRE(LockPluginCommandList);
  for (auto it = pluginCommandList.begin(); it != pluginCommandList.end();
       ++it) {
    const auto& currentCommand = *it;
    if (currentCommand.pluginHandle == pluginHandle &&
        !strcmp(currentCommand.command, command)) {
      pluginCommandList.erase(it);
      EXCLUSIVE_RELEASE();
      if (!dbgcmddel(command)) goto beach;
...
beach:
  dprintf(QT_TRANSLATE_NOOP(
              "DBG", "[PLUGIN, %s] Command \"%s\" failed to unregister...\n"),
          plugName.c_str(), command);
  return false;
}
...
int pluginmenuadd(int hMenu, const char* title) {
  if (!title || !strlen(title)) return -1;
...
\param hEntry The handle you like to have the entry. This should be a unique
value in the scope of the plugin that registered the \p hMenu. Cannot be -1.
\param title The menu entry title.
\return true if the \p hEntry was unique and the entry was successfully added,
false otherwise.
*/
bool pluginmenuaddentry(int hMenu, int hEntry, const char* title) {
  if (!title || !strlen(title) || hEntry == -1) return false;
  EXCLUSIVE_ACQUIRE(LockPluginMenuList);
  int pluginHandle = -1;
  // find plugin handle
  ...
  for (const auto& currentMenu : pluginMenuEntryList)
    if (currentMenu.pluginHandle == pluginHandle &&
        currentMenu.hEntryPlugin == hEntry)
      return false;
  int hNewEntry = GuiMenuAddEntry(hMenu, title);
  if (hNewEntry == -1) return false;
...
static void pluginmenuclear_helper(int hMenu) {
  ...
  for (auto& hMenu : menuClearQueue) pluginmenuclear_helper(hMenu);
}
...
\return The value the plugin told it to return. See plugin documentation for
more information.
*/
...
\param icon The icon (can be all kinds of formats).
*/
void pluginmenuentryseticon(int pluginHandle, int hEntry,
                            const ICONDATA* icon) {
  if (hEntry == -1) return;
  SHARED_ACQUIRE(LockPluginMenuList);
  for (const auto& currentMenu : pluginMenuEntryList) {
...
void pluginmenuentrysethotkey(int pluginHandle, int hEntry,
                              const char* hotkey) {
  if (hEntry == -1 || !hotkey) return;
  SHARED_ACQUIRE(LockPluginMenuList);
  for (const auto& currentMenu : pluginMenuEntryList) {
    if (currentMenu.pluginHandle == pluginHandle &&
        currentMenu.hEntryPlugin == hEntry) {
...
bool pluginmenuentryremove(int pluginHandle, int hEntry) {
  EXCLUSIVE_ACQUIRE(LockPluginMenuList);
  for (auto it = pluginMenuEntryList.begin(); it != pluginMenuEntryList.end();
       ++it) {
    const auto& currentEntry = *it;
    if (currentEntry.pluginHandle == pluginHandle &&
        currentEntry.hEntryPlugin == hEntry) {
...
bool pluginexprfuncregister(int pluginHandle, const char* name, int argc,
                            CBPLUGINEXPRFUNCTION cbFunction, void* userdata) {
  String plugName;
  if (!findPluginName(pluginHandle, plugName)) return false;
  ...
  EXCLUSIVE_RELEASE();
  dprintf(QT_TRANSLATE_NOOP(
              "DBG", "[PLUGIN, %s] Expression function \"%s\" registered!\n"),
          plugName.c_str(), name);
  return true;
}
...
bool pluginexprfuncunregister(int pluginHandle, const char* name) {
  ...
  if (!findPluginName(pluginHandle, plugName)) return false;
  EXCLUSIVE_ACQUIRE(LockPluginExprfunctionList);
  for (auto it = pluginExprfunctionList.begin();
       it != pluginExprfunctionList.end(); ++it) {
    const auto& currentExprfunction = *it;
    if (currentExprfunction.pluginHandle == pluginHandle &&
        !strcmp(currentExprfunction.name, name)) {
      pluginExprfunctionList.erase(it);
      EXCLUSIVE_RELEASE();
      if (!ExpressionFunctions::Unregister(name)) goto beach;
      dprintf(
          QT_TRANSLATE_NOOP(
              "DBG", "[PLUGIN, %s] Expression function \"%s\" unregistered!\n"),
          plugName.c_str(), name);
      return true;
    }
  }
...
bool pluginformatfuncregister(int pluginHandle, const char* type,
                              CBPLUGINFORMATFUNCTION cbFunction,
                              void* userdata) {
  String plugName;
  if (!findPluginName(pluginHandle, plugName)) return false;
  ...
  EXCLUSIVE_RELEASE();
  dprintf(QT_TRANSLATE_NOOP(
              "DBG", "[PLUGIN, %s] Format function \"%s\" registered!\n"),
          plugName.c_str(), type);
  return true;
}
...
bool pluginformatfuncunregister(int pluginHandle, const char* type) {
  ...
  if (!findPluginName(pluginHandle, plugName)) return false;
  EXCLUSIVE_ACQUIRE(LockPluginFormatfunctionList);
  for (auto it = pluginFormatfunctionList.begin();
       it != pluginFormatfunctionList.end(); ++it) {
    const auto& currentFormatfunction = *it;
    if (currentFormatfunction.pluginHandle == pluginHandle &&
        !strcmp(currentFormatfunction.name, type)) {
      pluginFormatfunctionList.erase(it);
      EXCLUSIVE_RELEASE();
      if (!FormatFunctions::Unregister(type)) goto beach;
      dprintf(QT_TRANSLATE_NOOP(
                  "DBG", "[PLUGIN, %s] Format function \"%s\" unregistered!\n"),
              plugName.c_str(), type);
      return true;
    }
  }
