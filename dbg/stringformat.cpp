...
#define QT_TRANSLATE_NOOP(s, e) s
...
static String printValue(FormatValueType value, ValueType type) {
  duint valuint = DbgValFromString(value);
  ...
  if (1) {
    switch (type) {
...
#if 0
        if (disasmgetstringatwrapper(valuint, string, false)) result = string;
#endif
        break;
      ...
      case ValueType::Instruction: {
        ...
        if (1 || !disasmfast(valuint, &info, true))
          result = "???";
        else
...
static const char* getArgExpressionType(const String& formatString,
                                        ValueType& type, String& complexArgs) {
  size_t toSkip = 0;
  type = ValueType::Hex;
  complexArgs.clear();
  if (formatString.size() > 2 && !isdigit(formatString[0]) &&
      formatString[1] == ':')  // simple type
  {
    if (!typeFromCh(formatString[0], type)) return nullptr;
    toSkip = 2;  // skip '?:'
  } else if (formatString.size() > 2 &&
             formatString.find('@') != String::npos)  // complex type
  {
...
static String handleFormatString(const String& formatString,
                                 const FormatValueVector& values) {
...
static String printComplexValue(FormatValueType value,
                                const String& complexArgs) {
#if 0
