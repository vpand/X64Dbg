...
static inline bool convertLongLongNumber(const char* str,
...
void StringUtils::Split(const String& s, char delim,
...
static int IsValidUTF8Char(const char* data, int size) {
  ...
  else if (*(unsigned char*)data >= 0xF0)  // 4 bytes
  {
    if (size < 4) return 0;
    for (int i = 1; i <= 3; i++) {
      if ((*(unsigned char*)(data + i) & 0xC0) != 0x80) return 0;
    }
    return 4;
  ...
  {
    if (size < 2) return 0;
    if ((*(unsigned char*)(data + 1) & 0xC0) != 0x80) return 0;
    return 2;
...
String StringUtils::Escape(const String& s, bool escapeSafe) {
  ...
  for (size_t i = 0; i < s.length(); i++) {
    ...
    switch (ch) {
      ...
      default:
        int UTF8CharSize;
        if (ch >= 0x80 && (UTF8CharSize = IsValidUTF8Char(
                               s.c_str() + i, int(s.length() - i))) !=
                              0)  // UTF-8 Character is emitted directly
        {
...
bool StringUtils::Unescape(const String& s, String& result, bool quoted) {
  ...
  size_t i = 0;
  auto nextChar = [&]() {
    if (i == s.length()) return mLastChar = EOF;
    ...
    if (mLastChar == '\\')  // escape sequence
    {
      ...
      else if (mLastChar == 'x')  //\xHH
      {
        ...
        if (isxdigit(ch1) && isxdigit(ch2)) {
          ...
          byteStr[0] = ch1;
          byteStr[1] = ch2;
          unsigned long long hexData;
          auto error = convertLongLongNumber(byteStr, hexData, 16);
          if (error)
            return false;  // convertNumber failed (%s) for hex sequence
                           // \"\\x%c%c\" in string literal
...
// https://stackoverflow.com/questions/216823/whats-the-best-way-to-trim-stdstring/16743707#16743707
...
String StringUtils::PadLeft(const String& s, size_t minLength, char ch) {
  ...
  for (size_t i = 0; i < pad.length(); i++) pad[i] = ch;
  return pad + s;
}
...
String StringUtils::Utf16ToUtf8(const wchar_t* wstr) {
  ...
  if (!wstr || !*wstr) return convertedString;
  auto requiredSize =
  ...
  if (requiredSize > 0) {
    ...
    if (!WideCharToMultiByte(CP_UTF8, 0, wstr, -1,
                             (char*)convertedString.c_str(), requiredSize,
                             nullptr, nullptr))
...
WString StringUtils::LocalCpToUtf16(const char* str) {
  ...
  if (requiredSize > 0) {
    convertedString.resize(requiredSize - 1);
    if (!MultiByteToWideChar(CP_ACP, 0, str, -1,
                             (wchar_t*)convertedString.c_str(), requiredSize))
      convertedString.clear();
  }
  return convertedString;
...
#if 0
  String convertedString;
  if (str.size() == 0) return convertedString;
  int requiredSize = WideCharToMultiByte(CP_ACP, 0, str.c_str(), -1, nullptr, 0,
                                         nullptr, nullptr);
  if (requiredSize > 0) {
    convertedString.resize(requiredSize - 1);
    if (!WideCharToMultiByte(CP_ACP, 0, str.c_str(), -1,
                             (char*)convertedString.c_str(), requiredSize,
                             nullptr, nullptr))
      convertedString.clear();
  }
...
void StringUtils::ReplaceAll(WString& s, const WString& from,
                             const WString& to) {
  size_t start_pos = 0;
  while ((start_pos = s.find(from, start_pos)) != std::string::npos) {
    s.replace(start_pos, from.length(), to);
...
String StringUtils::vsprintf(_In_z_ _Printf_format_string_ const char* format,
                             va_list args) {
#if 0
  char sbuffer[64] = "";
  if (_vsnprintf_s(sbuffer, _TRUNCATE, format, args) != -1) return sbuffer;
...
#else
  return QString::vasprintf(format, args).toUtf8().data();
#endif
}
...
WString StringUtils::vsprintf(
    _In_z_ _Printf_format_string_ const wchar_t* format, va_list args) {
#if 0
  wchar_t sbuffer[64] = L"";
  if (_vsnwprintf_s(sbuffer, _TRUNCATE, format, args) != -1) return sbuffer;
...
bool StringUtils::FromHex(const String& text, std::vector<unsigned char>& data,
                          bool reverse) {
  auto size = text.size();
...
String StringUtils::ToHex(const unsigned char* buffer, size_t size,
                          bool reverse) {
  String result;
...
String StringUtils::ToCompressedHex(const unsigned char* buffer, size_t size) {
  ...
  for (size_t i = 0; i < size;) {
    ...
    for (; i < size && buffer[i] == lastCh; i++) repeat++;
    if (repeat == 2) {
      result.push_back(HEXLOOKUP[(lastCh >> 4) & 0xF]);
      result.push_back(HEXLOOKUP[lastCh & 0xF]);
    } else if (repeat > 2)
...
bool StringUtils::FromCompressedHex(const String& text,
  ...
  for (size_t i = 0; i < size;) {
    ...
    if (text[i] == '{') {
      ...
      size_t repeat = 0;
      if (!convertNumber(repeatStr.c_str(), repeat, 16) ||
          !repeat)  // conversion failed or repeat zero times
        return false;
      for (size_t j = 1; j < repeat; j++) data.push_back(lastCh);
    }
  }
  return true;
}
