...
/*
 * The tables below were generated for each OS version using a modified version
 * of wscg64 (https://github.com/hfiref0x/SyscallTables). They were then
 * converted to the format below to save space. std::map and std::string are
 * banned in this file, they add about 600KB. The syscalls are sorted by name
 * only, so lookup is O(N) and done with string comparisons. This is okay
 * because lookups are rare.
 *
 * All OS versions have separate syscall tables for x86 and x64, except 2600
 * which only exists as x86. WOW64 processes use the x64 table.
 *
 * Versions 14393 and later are not included because they come with win32u.dll,
 * which exports win32k syscalls like ntdll does for ntoskrnl.
 */
...
typedef struct _WIN32K_SYSCALL_INFO {
  ...
  // A value of -1 means Win32k does not have the syscall on this OS/bitness
  // combination.
  int GetSyscallIndex(USHORT osBuildNumber, bool nativeX86) const {
    return (
        int)(osBuildNumber == 2600
                 ? s.Index2600
