...
MainWindow::MainWindow(QWidget* parent)
  ...
  // Default to python3 commander
  char savedType[MAX_SETTING_SIZE] = "";
  if (!BridgeSettingGet("Gui", "ScriptType", savedType)) {
    BridgeSettingSet("Gui", "ScriptType", "Python3");
  }
  ...
  if (BridgeIsProcessElevated()) {
    ...
    mWindowMainTitle = tr("%1 v%2" STUDIO_BUILD_INFO)
  ...
  // Decompiler view
  mDecompView = new UrDecompiler();
  mDecompView->setWindowTitle(tr("UrDecompiler"));
  mDecompView->setWindowIcon(DIcon("compile.png"));
  mDecompView->hide();
  mDecompView->setEnabled(STUDIO_ENABLE_DECOMPILER);

  // Symbol view
  mVMPSymbolView = new VMPSymbolView();
  mVMPSymbolView->setWindowTitle(tr("PhoneVMP"));
  mVMPSymbolView->setWindowIcon(DIcon("functions.png"));
  mVMPSymbolView->hide();
  mVMPSymbolView->setEnabled(STUDIO_ENABLE_PHONEVMP);

  // Source view
  mSourceViewManager = nullptr;  // new SourceViewerManager();
#if 0
  ...
  // Breakpoints
  mBreakpointsView = nullptr;  // new BreakpointsView();
#if 0
  ...
  // Callstack view
  mCallStackView = nullptr;  // new CallStackView();
#if 0
  ...
  // CPU view
  mCpuWidget = new CPUWidgetFile(this);
  mCpuWidget->setWindowTitle(tr("LightIDA"));
#ifdef _WIN64
  ...
  // Thread view
  mThreadView = nullptr;  // new ThreadView();
#if 0
  ...
  mTraceWidget->setWindowTitle(tr("UraniumVM"));
  ...
  connect(mTraceWidget->getTraceBrowser(), SIGNAL(displayLogWidget()), this,
          SLOT(displayLogWidget()));
  mTraceWidget->setEnabled(STUDIO_ENABLE_URANIUMVM);
  ...
  mWidgetList.push_back(WidgetInfo(mDecompView, "DecompilerTab"));
  mWidgetList.push_back(WidgetInfo(mVMPSymbolView, "SymbolsTab"));
  ...
  makeCommandAction(ui->actionSave_Database, "save");
  makeCommandAction(ui->actionSave_File, "savefile");
  makeCommandAction(ui->actionClose, "stop");
  makeCommandAction(ui->actionClose_2, "stop");
  makeCommandAction(ui->actionReadobj, "readobj");
  ...
  makeCommandAction(ui->actionObjcDump, "readobjc");
  makeCommandAction(ui->actionSwiftDump, "readswift");
  makeCommandAction(ui->actionExports, "list_export");
  makeCommandAction(ui->actionImports, "list_import");
  makeCommandAction(ui->actionModules, "list_module");
  connect(ui->actionCpu, SIGNAL(triggered()), this,
          SLOT(displayCpuWidgetShowCpu()));
  connect(ui->actionSymbolInfo, SIGNAL(triggered()), this,
          SLOT(displaySymbolWidget()));
  connect(ui->actionUraniumVM, SIGNAL(triggered()), this,
          SLOT(displayRunTrace()));
  connect(ui->actionDecompiler, SIGNAL(triggered()), this,
          SLOT(displayDecompWidget()));
  connect(ui->actionPhoneVMP, SIGNAL(triggered()), this,
          SLOT(displaySymbolWidget()));
  connect(ui->actionSource, SIGNAL(triggered()), this,
          SLOT(displaySourceViewWidget()));
  // connect(mVMPSymbolView, SIGNAL(showReferences()), this,
  // SLOT(displayReferencesWidget()));
  ...
  connect(ui->actionCalculator, SIGNAL(triggered()), this,
          SLOT(openCalculator()));
  connect(ui->actionPatchFile, SIGNAL(triggered()), this, SLOT(patchWindow()));
  connect(ui->actionPatches_2, SIGNAL(triggered()), this,
          SLOT(displayPatches()));
  connect(ui->actionDisasm_file, SIGNAL(triggered()), this,
          SLOT(disasmWindow()));
  ...
  connect(ui->actionUVMAttach, SIGNAL(triggered()), this,
          SLOT(displayAttach()));
  makeCommandAction(ui->actionUVMDetach, "uvmdetach");
  connect(ui->actionLaunch_APK, SIGNAL(triggered()), this,
  ...
  connect(ui->actionVSPManager, SIGNAL(triggered()), this,
          SLOT(managePlugin()));
...
void MainWindow::closeEvent(QCloseEvent* event) {
  if (0 && DbgIsDebugging() && ConfigBool("Gui", "ShowExitConfirmation")) {
...
void MainWindow::saveWindowSettings() {
  ...
  // mVMPSymbolView->saveWindowSettings();
}
...
void MainWindow::loadWindowSettings() {
  ...
  // mVMPSymbolView->loadWindowSettings();
}
...
void MainWindow::openFileSlot() {
  auto filename = QFileDialog::getOpenFileName(
      ...
      tr("All files (*);;Libraries (*.so *.dylib);;VDB (*.vdb)"));
...
void MainWindow::displayDecompWidget() { showQWidgetTab(mDecompView); }

void MainWindow::displaySymbolWidget() { showQWidgetTab(mVMPSymbolView); }
...
void MainWindow::blog() {
  QDesktopServices::openUrl(
      QUrl("https://www.reddit.com/user/JesseVPAND/"));
}

void MainWindow::reportBug() {
  QDesktopServices::openUrl(QUrl("https://github.com/vpand/X64Dbg/issues"));
}
...
void MainWindow::dbgStateChangedSlot(DBGSTATE state) {
  if (state == paused) {
    ...
    mCpuWidget->getDisasmWidget()->resetDatabase(mdb);
    mCpuWidget->getDumpWidget()->resetDatabase(mdb);
    mCpuWidget->reloadFunc();

    auto winSect = mCpuWidget->getSectWidget();
    winSect->setRowCount(mdb->db->sections().size());
    auto sit = mdb->db->sections().begin();
    for (int i = 0; i < (int)mdb->db->sections().size(); i++, sit++) {
      auto sect = &sit->second;
      char start[32], end[32];
      sprintf(start, "%016llX", sect->addr);
      sprintf(end, "%016llX", sect->addr + sect->size);
      winSect->setCellContent(i, 0, start);
      winSect->setCellContent(i, 1, end);
      winSect->setCellContent(i, 2, sect->name.data());
    }
    winSect->reloadData();

    GuiAddLogMessage(QString("Constructed memory database from %1, total has:\n"
                             "%2 segments, %3 functions, %4 strings;\n")
                         .arg(mdb->path + (mdb->isvdb ? "[vdb]" : ""))
                         .arg(mdb->db->sections().size())
                         .arg(mdb->db->functions().size())
                         .arg(mdb->strs.size())
                         .toUtf8()
                         .data());
  } else if (state == stopped) {
    mCpuWidget->getDisasmWidget()->resetDatabase(nullptr);
    mCpuWidget->getDumpWidget()->resetDatabase(nullptr);

    auto winFn = mCpuWidget->getSidebarWidget();
    winFn->setRowCount(0);
    winFn->reloadData();

    auto winSect = mCpuWidget->getSectWidget();
    winSect->setRowCount(0);
    winSect->reloadData();

    auto winStr = mCpuWidget->getStackWidget();
    winStr->setRowCount(0);
    winSect->reloadData();

    GuiAddLogMessage("Closed current database.\n");
  }
}

void MainWindow::on_actionFaq_triggered() {
  QDesktopServices::openUrl(
      QUrl("https://github.com/vpand/vsusermanual"));
}
...
void MainWindow::clickPluginMenu() {
  ...
  VSPluginInstance& adp = VSPluginManager::inst()->vsps[data.toInt()];
  adp.invoke(vsp_event_main_menu);
}

void MainWindow::addPluginMenu() {
  int iconid = 0;
  for (auto& adp : VSPluginManager::inst()->vsps) {
    ...
    newAction->setData(
        QVariant((int)(&adp - &VSPluginManager::inst()->vsps[0])));
...
void MainWindow::onMenuCustomized() {
  ...
  menus << ui->menuFile << ui->menuOptions << ui->menuHelp << ui->menuView;
  menuNativeNames << "File"
                  << "Option"
                  << "Help"
                  << "View";
  menuTextStrings << &mFileMenuStrings << &mOptionsMenuStrings
                  << &mHelpMenuStrings << &mViewMenuStrings;
...
void MainWindow::on_actionPlugins_triggered() {
  QDesktopServices::openUrl(QUrl("https://plugins.vpand.com"));
}
...
VSPluginView::VSPluginView() : StdTable(nullptr) {
  setDrawDebugOnly(false);
  setWindowTitle("VSPlugin Manager");
  ...
  auto adpmgr = VSPluginManager::inst();
  setRowCount(adpmgr->vsps.size());
  for (size_t i = 0; i < adpmgr->vsps.size(); i++) {
    VSPluginInstance *adp = &adpmgr->vsps[i];
...
void VSPluginView::enableSlot() {
  int sel = getInitialSelection();
  VSPluginInstance *adp = (VSPluginInstance *)getCellUserdata(sel, 0);
  if (!adp->path.length()) {
    return;
  }
  adp->enable = true;
  setCellContent(sel, 1, "On");
  BridgeSettingSetUint(CFGSECT_VSP, adp->name().data(), 1);
  reloadData();
}

void VSPluginView::disableSlot() {
  int sel = getInitialSelection();
  VSPluginInstance *adp = (VSPluginInstance *)getCellUserdata(sel, 0);
  if (!adp->path.length()) {
    return;
  ...
  BridgeSettingSetUint(CFGSECT_VSP, adp->name().data(), 0);
  reloadData();
  adp->enable = false;
}

void VSPluginView::deleteSlot() {
  int sel = getInitialSelection();
  VSPluginInstance *adp = (VSPluginInstance *)getCellUserdata(sel, 0);
  if (!adp->path.length()) {
    return;
  ...
  if (file.remove()) {
    ...
    BridgeSettingSetUint(CFGSECT_VSP, adp->name().data(), 1);  // for future load
  } else {
    GuiAddLogMessage(
...
void VSPluginView::contextMenuSlot(const QPoint &pos) {
  if (!VSPluginManager::inst()->vsps.size()) {
    return;
  }
