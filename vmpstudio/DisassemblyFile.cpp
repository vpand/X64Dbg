...
DisassemblyFile::DisassemblyFile(QWidget* parent, bool isMain)
    : AbstractTableView(parent), mIsMain(isMain) {
  mMemPage = new VMPOpcodePage(0, 0);
  ...
  mGuiState = DisassemblyFile::NoState;
  ...
  addColumnAt(getCharWidth() * 2 * 8 + 0, "", false);              // bytes
  addColumnAt(getCharWidth() * 50, "", false);              // disassembly
  addColumnAt(getCharWidth() * MAX_LABEL_SIZE, "", false);  // comments
  ...
  // Slots
  connect(Bridge::getBridge(), SIGNAL(updateDisassembly()), this,
          SLOT(reloadData()));
  connect(Bridge::getBridge(), SIGNAL(dbgStateChanged(DBGSTATE)), this,
          SLOT(debugStateChangedSlot(DBGSTATE)));
  connect(this, SIGNAL(selectionChanged(dsint)), this,
          SLOT(selectionChangedSlot(dsint)));
  connect(Config(), SIGNAL(tokenizerConfigUpdated()), this,
          SLOT(tokenizerConfigUpdatedSlot()));
  connect(Config(), SIGNAL(guiOptionsUpdated()), this,
          SLOT(updateConfigSlot()));

  Initialize();
}

DisassemblyFile::~DisassemblyFile() {
  delete mMemPage;
  delete mDisasm;
  if (mXrefInfo.refcount != 0) BridgeFree(mXrefInfo.references);
}

void DisassemblyFile::updateColors() {
  ...
  mModifiedBytesBackgroundColor =
      ConfigColor("DisassemblyModifiedBytesBackgroundColor");
  mRestoredBytesColor = ConfigColor("DisassemblyRestoredBytesColor");
  mRestoredBytesBackgroundColor =
      ConfigColor("DisassemblyRestoredBytesBackgroundColor");
  ...
  mUnconditionalJumpLineColor =
      ConfigColor("DisassemblyUnconditionalJumpLineColor");
  mConditionalJumpLineTrueColor =
      ConfigColor("DisassemblyConditionalJumpLineTrueColor");
  mConditionalJumpLineFalseColor =
      ConfigColor("DisassemblyConditionalJumpLineFalseColor");
...
void DisassemblyFile::updateFonts() {
  setFont(ConfigFont("Disassembly"));
  invalidateCachedFont();
}

void DisassemblyFile::updateConfigSlot() {
  setDisassemblyPopupEnabled(
      !Config()->getBool("Disassembler", "NoBranchDisasmPreview"));
}

void DisassemblyFile::tokenizerConfigUpdatedSlot() {
  mDisasm->UpdateConfig();
...
#define DbgGetFunctionTypeAt(addr) A64DbgGetFunctionTypeAt(this, addr)
#define DbgGetXrefTypeAt(addr) A64DbgGetXrefTypeAt(this, addr)
#define DbgGetLoopTypeAt(addr, depth) \
  A64DbgGetLoopTypeAt(this, nullptr, addr, depth)
#define GetCommentFormat(addr, comment, autocmm) \
  A64DbgGetCommentFormat(this, nullptr, addr, comment, autocmm)

static FUNCTYPE A64DbgGetFunctionTypeAt(DisassemblyFile* widget, duint addr) {
  addr = widget->vaToRva(addr);
  auto func = widget->rvaToFunction(addr, false);
  if (!func) return FUNC_NONE;
  duint start = func.rvastart;
  duint end = start + (func.func->insns.size() ? func.func->insns.rbegin()->fnoff : 0);
  if (func.func->insns.size() == 1)
    return FUNC_SINGLE;
  else if (addr == start)
    return FUNC_BEGIN;
  else if (addr >= end)
...
static XREFTYPE A64DbgGetXrefTypeAt(DisassemblyFile* widget, duint addr) {
  addr = widget->vaToRva(addr);
  if (widget->isDstRva(addr)) {
    return XREF_JMP;
  }
  auto func = widget->rvaToFunction(addr, false);
  if (!func) return XREF_NONE;
  if (addr == func.rvastart) return XREF_CALL;
  return XREF_NONE;
}

static void find_loop(const VMPManaFunc* func, const mana::Insinfo* insn,
                      const mana::Loops& loop, int depth, duint addr,
                      duint& start, duint& end) {
  uint frva = (uint)(addr - func->rvastart);

  // check whether is loop start
  for (auto& l : loop) {
    auto insnstart = insn + l.second;
    auto insnend = insn + l.first;
    if (insnstart->fnoff == frva) {
      start = func->rvastart + insnstart->fnoff;
      end = func->rvastart + insnend->fnoff;
      return;
    }
  }

  // check whether is loop end
  for (auto& l : loop) {
    auto insnstart = insn + l.second;
    auto insnend = insn + l.first;
    if (frva == insnend->fnoff) {
      start = func->rvastart + insnstart->fnoff;
      end = func->rvastart + insnend->fnoff;
      return;
    }
  ...
  // check whther is loop body
  int tmpdepth = 0;
  for (auto& l : loop) {
    auto insnstart = insn + l.second;
    auto insnend = insn + l.first;
    if (insnstart->fnoff < frva && frva < insnend->fnoff + insn->info.oplen) {
      if (tmpdepth < depth) {
        tmpdepth++;
        continue;
      }
...
static LOOPTYPE A64DbgGetLoopTypeAt(DisassemblyFile* widget,
                                    mana::Binary* manadb, duint addr,
                                    int depth) {
  addr = widget->vaToRva(addr);
  // render on sidebar, so always return none type
  if (1 || (addr & 1) || depth > 0) return LOOP_NONE;

  auto func = widget->rvaToFunction(addr, false);
  if (!func || !func.func->loops.size()) return LOOP_NONE;

  duint start = 0;
  duint end = 0;
  auto insn = &func.func->insns[0];
  find_loop(&func, insn, func.func->loops, depth, addr, start, end);
  if (!start && !end) {
    return LOOP_NONE;
  }
  if (start == end || func.func->loops.size() == 1)
    return LOOP_SINGLE;
  else if (addr == start)
    return LOOP_BEGIN;
  else if (addr == end)
...
static bool A64DbgGetCommentFormat(DisassemblyFile* widget,
                                   ManaDatabase* manadb, duint addr,
                                   QString& comment, bool* autoComment) {
  auto func = widget->rvaToFunction(widget->vaToRva(addr), true);
  if (!func) {
    char text[MAX_COMMENT_SIZE];
    if (DbgGetCommentAt(manadb, addr, text)) {
      comment = text;
      autoComment[0] = false;
      return true;
    }
  ...
  comment = QString(func.func->name.data());
  autoComment[0] = true;
  return true;
}

/**
 * @brief       This method has been reimplemented. It returns the string to
 * paint or paints it by its own.
 *
 ...
 */
QString DisassemblyFile::paintContent(QPainter* painter, dsint rowBase,
                                      int rowOffset, int col, int x, int y,
                                      int w, int h) {
  Q_UNUSED(rowBase);
  ...
  switch (col) {
    case 0:  // Draw address (+ label)
    {
      ...
      bool isbookmark = DbgGetBookmarkAt((ManaDatabase*)mManaDB, cur_addr);
      if (rvaToVa(mInstBuffer.at(rowOffset).rva) == DbgValFromString("pc") &&
          !Bridge::getBridge()->mIsRunning &&
          DbgMemFindBaseAddr(DbgValFromString("pc"),
                             nullptr))  // cip + not running + valid cip
      {
        ...
        if (!isbookmark)  // no bookmark
        {
          if (bpxtype & bp_normal)  // normal breakpoint
          {
            ...
            if (bpColor == mCipBackgroundColor) bpColor = mCipColor;
          ...
          } else if (bpxtype & bp_hardware)  // hardware breakpoint only
          {
            ...
            if (hwbpColor == mCipBackgroundColor) hwbpColor = mCipColor;
        ...
        } else  // bookmark
        {
          ...
          if (bookmarkColor == mCipBackgroundColor) bookmarkColor = mCipColor;
    ...
    case 1:  // draw bytes
    {
      ...
      // draw jump arrows
      Instruction_t::BranchType branchType =
          mInstBuffer.at(rowOffset).branchType;
      int jumpsize = paintJumpsGraphic(
          painter, x + funcsize, y - 1, wRVA,
          branchType != Instruction_t::None &&
              branchType != Instruction_t::Call);  // jump line
    ...
    case 2:  // draw disassembly (with colours needed)
    {
      ...
      while (1)  // paint all loop depths
      {
        ...
        LOOPTYPE loopLast = DbgGetLoopTypeAt(
      ...
      int xinc = 4;
      RichTextPainter::paintRichText(
          painter, x + loopsize, y, getColumnWidth(col) - loopsize,
          getRowHeight(), xinc, richText, mFontMetrics);
    ...
    case 3:  // draw comments
    {
      ...
      ARGTYPE argLast =
          DbgGetArgTypeAt(cur_addr + mInstBuffer.at(rowOffset).length - 1);
      ...
      int argsize = funcType == Function_none
                        ? 3
                        : paintFunctionGraphic(painter, x, y, funcType, false);
      ...
      if (GetCommentFormat(cur_addr, comment, &autoComment)) {
        ...
        int width = mFontMetrics->width(comment) * 2;
        if (width > w) width = w;
        if (width)
          painter->fillRect(QRect(x + argsize, y, width, h),
                            QBrush(backgroundColor));  // fill comment color
        painter->drawText(QRect(x + argsize, y, width, h),
      ...
      if (mShowMnemonicBrief) {
        ...
        QString mnem;
        for (const ZydisTokenizer::SingleToken& token :
             mInstBuffer.at(rowOffset).tokens.tokens) {
        ...
        if (mnem.isEmpty()) mnem = mInstBuffer.at(rowOffset).instStr;
...
************************************************************************************/
/**
 * @brief       This method has been reimplemented. It manages the following
 * actions:
 ...
 * @return      Nothing.
 */
void DisassemblyFile::mouseMoveEvent(QMouseEvent* event) {
  // qDebug() << "DisassemblyFile::mouseMoveEvent";

  bool wAccept = true;
  int y = event->y();

  if (mGuiState == DisassemblyFile::MultiRowsSelectionState) {
...
duint DisassemblyFile::getDisassemblyPopupAddress(int mousex, int mousey) {
  if (mHighlightingMode) return 0;  // Don't show this in highlight mode
  ...
  if (rowOffset < mInstBuffer.size()) {
    ...
    if (ZydisTokenizer::TokenFromX(instruction.tokens, token, mousex,
                                   mFontMetrics)) {
      duint addr = token.value.value;
      bool isCodePage = true;  // DbgFunctions()->MemIsCodePage(addr, false);
      ...
      if (isCodePage && (addr - mMemPage->getBase() < mInstBuffer.front().rva ||
                         addr - mMemPage->getBase() > mInstBuffer.back().rva)) {
        return addr;
      }
    }
  }
 ...
 * @return      Nothing.
 */
void DisassemblyFile::mousePressEvent(QMouseEvent* event) {
  bool wAccept = false;
  ...
  if (mHighlightingMode || mPermanentHighlightingMode) {
    if (getColumnIndexFromX(event->x()) == 2)  // click in instruction column
    {
      ...
      if (rowOffset < mInstBuffer.size()) {
        ZydisTokenizer::SingleToken token;
        if (ZydisTokenizer::TokenFromX(mInstBuffer.at(rowOffset).tokens, token,
                                       event->x(), mFontMetrics)) {
    ...
    if (!mPermanentHighlightingMode) return;
  }

  if (DbgIsDebugging() && ((event->buttons() & Qt::LeftButton) != 0) &&
    ...
    if (getGuiState() == AbstractTableView::NoState) {
      if (event->y() > getHeaderHeight()) {
        ...
        if (mInstBuffer.size() > wIndex && wIndex >= 0) {
          ...
          {
            setSingleSelection(getInitialSelection());
            expandSelectionUpTo(getInstructionRVA(getInitialSelection(), 1) -
                                1);
            expandSelectionUpTo(wRowIndex);
          } else  // select down
          {
          ...
          mGuiState = DisassemblyFile::MultiRowsSelectionState;
 ...
 * @return      Nothing.
 */
void DisassemblyFile::mouseReleaseEvent(QMouseEvent* event) {
  bool wAccept = true;
...
************************************************************************************/
/**
 * @brief       This method has been reimplemented. It processes the Up/Down key
 * events.
 *
 ...
 * @return      Nothing.
 */
void DisassemblyFile::keyPressEvent(QKeyEvent* event) {
  int key = event->key();
  ...
  if (event->modifiers() == (Qt::ControlModifier | Qt::AltModifier)) {
    ...
    if (key == Qt::Key_Up)
      selectPrevious(expand);  // TODO: fix this shit to actually go to whatever
                               // the previous instruction shows
    else
      selectNext(expand);

    bool expandedUp = initialStart != getSelectionStart();
    dsint modifiedSelection =
        expandedUp ? getSelectionStart() : getSelectionEnd();

    if (modifiedSelection < botRVA) {
      setTableOffset(modifiedSelection);
    } else if (modifiedSelection >= topRVA) {
      setTableOffset(
          getInstructionRVA(modifiedSelection, -getNbrOfLineToPrint() + 2));
    }
    ...
    duint dest = DbgGetBranchDestination((ManaDatabase*)mManaDB,
 ...
 ***********************************************************************************/
/**
 * @brief       This method has been reimplemented. It realigns the slider on
 * real instructions except when the type is QAbstractSlider::SliderNoAction.
 * This type (QAbstractSlider::SliderNoAction) is used to force the
 * disassembling at a specific RVA.
 *
 ...
 */
dsint DisassemblyFile::sliderMovedHook(int type, dsint value, dsint delta) {
  ShowDisassemblyPopup(0, 0, 0);

  // QAbstractSlider::SliderNoAction is used to disassembe at a specific address
  if (type == QAbstractSlider::SliderNoAction) return value + delta;

  // If it's a slider action, disassemble one instruction back and one
  // instruction next in order to be aligned on a real instruction
 ...
 * @return      Nothing.
 */
int DisassemblyFile::paintJumpsGraphic(QPainter* painter, int x, int y,
                                       dsint addr, bool isjmp) {
  ...
  if (branchType != Instruction_t::None && branchType != Instruction_t::Call) {
    ...
    dsint destVA =
        DbgGetBranchDestination((ManaDatabase*)mManaDB, rvaToVa(selHeadRVA));
    ...
    // TODO: bad performance for sure, this code is also doing things in a super
    // weird order...
    duint max = selVa, min = selVa;
    showXref = true;
    int jmpcount = 0;
    for (duint i = 0; i < mXrefInfo.refcount; i++) {
      if (mXrefInfo.references[i].type != XREF_JMP) continue;
      jmpcount++;
      if (curVa == mXrefInfo.references[i].addr) wPict = GD_VertHori;
    ...
    if (jmpcount) {
      ...
      if (wPict == GD_Nothing && curVa > min && curVa < max) wPict = GD_Vert;
    }
  }
  ...
  if (isjmp) {
    duint curInstDestination =
        DbgGetBranchDestination((ManaDatabase*)mManaDB, curVa);
...
int DisassemblyFile::paintFunctionGraphic(QPainter* painter, int x, int y,
 ...
 ***********************************************************************************/
/**
 * @brief       Returns the RVA of count-th instructions before the given
 * instruction RVA.
 *
 ...
 */
dsint DisassemblyFile::getPreviousInstructionRVA(dsint rva, duint count) {
#if 0
    QByteArray wBuffer;
...
#else
  dsint rvastart =
      mManaDB->db->functions().begin()->first - mManaDB->db->imageBase();
  if (rva <= rvastart) {
    return rvastart;
  }
  if (mManaDB->db->archType() == mana::ARM64) {
    return rva - 4 * count;
  }

  auto func = rvaToFunction(rva, false);
  if (!func) {
    return rva;
  }
  auto insn = mManaDB->rvaToInstruction(&func, rva);
  if (!insn) {
    return rva;
  }
  auto insnstart = &func.func->insns[0];
  if (insn - count >= insnstart) {
    return func.rvastart + insn[-count].fnoff;
  } else {
    func = rvaToFunction(func.rvastart - 1, false);
    if (!func) {
      return rva;
    }
    return func.rvastart + func.func->insns.rbegin()->fnoff;
  }
#endif
}

/**
 * @brief       Returns the RVA of count-th instructions after the given
 * instruction RVA.
 *
 * @param[in]   rva         Instruction RVA
 * @param[in]   count       Instruction count
 ...
 */
dsint DisassemblyFile::getNextInstructionRVA(dsint rva, duint count,
                                             bool isGlobal) {
#if 0
    QByteArray wBuffer;
...
#else
  dsint rvaend =
      mManaDB->db->functions().rbegin()->second.end - mManaDB->db->imageBase();
  if (rva >= rvaend) {
    return rvaend;
  }
  if (mManaDB->db->archType() == mana::ARM64) {
  ...
  auto insnend = &*func.func->insns.rbegin();
  if (insn + count <= insnend) {
    return func.rvastart + insn[count].fnoff;
  } else {
    return func.rvaend;
  }
...
/**
 * @brief       Returns the RVA of count-th instructions before/after (depending
 * on the sign) the given instruction RVA.
 *
 * @param[in]   rva         Instruction RVA
 * @param[in]   count       Instruction count
 *
 * @return      RVA of count-th instructions before/after the given instruction
 * RVA.
 */
dsint DisassemblyFile::getInstructionRVA(dsint index, dsint count) {
  dsint wAddr = 0;
  if (!mManaDB) return 0;

  if (count == 0) wAddr = index;
  if (count < 0)
 ...
 */
Instruction_t DisassemblyFile::DisassembleAt(dsint rva) {
#if 0
    if(mMemPage->getSize() < (duint)rva)
...
#else
  if (!mManaDB) return Instruction_t();
  switch (mManaDB->db->archType()) {
    case mana::ARM:
      rva -= rva % 2;  // must 2 bytes align
      break;
    case mana::ARM64:
      rva -= rva % 4;  // must 4 bytes align
      break;
  }

  uint32_t opcode[4];
  mMemPage->read(&opcode[0], rva, sizeof(opcode));
  auto insn =
      mDisasm->DisassembleAt((byte_t*)&opcode, sizeof(opcode), mManaDB->rtbase,
                             rva, false, (ManaDatabase*)mManaDB);
  if (insn.branchDestination) {
    mDstRvas.insert(
        {(dsint)rva, (dsint)(insn.branchDestination - mManaDB->rtbase)});
  }
  return insn;
#endif
}
...
/**
 * @brief       Disassembles the instruction count instruction afterc the
 * instruction at the given RVA. Count can be positive or negative.
 *
 ...
 * @return      Return the disassembled instruction.
 */
Instruction_t DisassemblyFile::DisassembleAt(dsint rva, dsint count) {
...
************************************************************************************/
void DisassemblyFile::expandSelectionUpTo(dsint to) {
...
void DisassemblyFile::setSingleSelection(dsint index) {
...
dsint DisassemblyFile::getInitialSelection() const {
  return mSelection.firstSelectedIndex;
}

dsint DisassemblyFile::getSelectionSize() const {
  return mSelection.toIndex - mSelection.fromIndex + 1;
}

dsint DisassemblyFile::getSelectionStart() const {
  return mSelection.fromIndex;
}

dsint DisassemblyFile::getSelectionEnd() const { return mSelection.toIndex; }

void DisassemblyFile::selectionChangedSlot(dsint Va) {
  if (mXrefInfo.refcount != 0) {
    BridgeFree(mXrefInfo.references);
    mXrefInfo.refcount = 0;
  }
  if (DbgIsDebugging()) DbgXrefGet(Va, &mXrefInfo);
}

void DisassemblyFile::selectNext(bool expand) {
  dsint wAddr;
  dsint wStart = getInstructionRVA(getSelectionStart(), 1) - 1;
  dsint wInstrSize;
  if (expand) {
    if (getSelectionEnd() == getInitialSelection() &&
        wStart != getSelectionEnd())  // decrease down
    {
...
void DisassemblyFile::selectPrevious(bool expand) {
  dsint wAddr;
  dsint wStart;
  dsint wInstrSize;
  wStart = getInstructionRVA(getSelectionStart(), 1) - 1;
  if (expand) {
    if (getSelectionStart() == getInitialSelection() &&
        wStart != getSelectionEnd())  // decrease up
    {
...
bool DisassemblyFile::isSelected(dsint base, dsint offset) {
  dsint wAddr = base;
...
bool DisassemblyFile::isSelected(QList<Instruction_t>* buffer,
                                 int index) const {
  if (buffer->size() > 0 && index >= 0 && index < buffer->size()) {
    if ((dsint)buffer->at(index).rva >= mSelection.fromIndex &&
        (dsint)buffer->at(index).rva <= mSelection.toIndex)
      return true;
    else
      return false;
  } else {
...
duint DisassemblyFile::getSelectedVa() const {
...
void DisassemblyFile::prepareDataCount(const QList<dsint>& wRVAs,
...
void DisassemblyFile::prepareDataRange(
    dsint startRva, dsint endRva,
    const std::function<bool(int, const Instruction_t&)>& disassembled) {
  dsint wAddrPrev = startRva;
  dsint wAddr = wAddrPrev;

  int i = 0;
  while (true) {
    if (wAddr > endRva) break;
    wAddrPrev = wAddr;
    auto wInst = DisassembleAt(wAddr);
    wAddr = getNextInstructionRVA(wAddr, 1);
    if (wAddr == wAddrPrev) break;
    if (!disassembled(i++, wInst)) break;
  }
}

RichTextPainter::List DisassemblyFile::getRichBytes(const Instruction_t& instr,
  ...
  for (size_t i = 0; i < richBytes.size(); i++) {
    ...
    if (0 && DbgFunctions()->ModRelocationAtAddr(byteAddr, &relocInfo)) {
      bool prevInSameReloc =
          relocInfo.rva < byteAddr - DbgFunctions()->ModBaseFromAddr(byteAddr);
    ...
    if (0 && isReal && DbgFunctions()->PatchGetEx(byteAddr, &patchInfo)) {
    ...
    if (curByte.textBackground.alpha() == 0) {
      auto byteSelected =
          byteAddr >= selectionFromVa && byteAddr <= selectionToVa;
...
void DisassemblyFile::prepareData() {
...
void DisassemblyFile::reloadData() {
...
/************************************************************************************
                        Public Methods
************************************************************************************/
duint DisassemblyFile::rvaToVa(dsint rva) const {
  if (!mManaDB) return 0;
  return (duint)mManaDB->rtbase + rva;
}

duint DisassemblyFile::vaToRva(duint va) const {
  if (!mManaDB) return 0;
  return va - (duint)mManaDB->rtbase;
}

void DisassemblyFile::gotoAddress(duint addr) {
...
void DisassemblyFile::disassembleAt(dsint parVA, bool history,
                                    dsint newTableOffset) {
  if (!mManaDB || !mManaDB->db->functions().size()) {
    mInstBuffer.clear();
    return;
  }

  duint wSize = mManaDB->db->functions().rbegin()->second.end -
                mManaDB->db->functions().begin()->first;
  dsint wBase = (dsint)mManaDB->rtbase;

  parVA =
      std::max(parVA, (dsint)(wBase + mManaDB->db->functions().begin()->first -
                              mManaDB->db->imageBase()));
  ...
  if (history) {
    ...
    if (mVaHistory.size() &&
        mCurrentVa < mVaHistory.size() - 1)  // mCurrentVa is not the last
    ...
    dsint selectionVA = rvaToVa(getInitialSelection());  // currently selected
                                                         // VA
    dsint selectionTableOffset = getTableOffset();
    if (selectionVA && mVaHistory.size() &&
        mVaHistory.last().va !=
            selectionVA)  // do not have 2x the same va in a row
    {
      mCurrentVa++;
  ...
  {
    // Update table offset depending on the location of the instruction to
    // disassemble
    if (mInstBuffer.size() > 0 && wRVA >= (dsint)mInstBuffer.first().rva &&
        wRVA < (dsint)mInstBuffer.last().rva) {
      int wI;
      bool wIsAligned = false;

      // Check if the new RVA is aligned on an instruction from the cache
      // (buffer)
  ...
  if(history)
  {
      ...
      for(int i=0; i<mVaHistory.size(); i++)
          strList += QString().sprintf("[%d]:%p,%p\n", i, mVaHistory.at(i).va,
  mVaHistory.at(i).tableOffset); MessageBoxA(GuiGetWindowHandle(),
  strList.toUtf8().constData(), QString().sprintf("mCurrentVa=%d",
  mCurrentVa).toUtf8().constData(), MB_ICONINFORMATION);
  }
  */
}

QList<Instruction_t>* DisassemblyFile::instructionsBuffer() {
  return &mInstBuffer;
}

void DisassemblyFile::disassembleAtSlot(dsint parVA, dsint parCIP) {
  if (mCodeFoldingManager) {
    mCodeFoldingManager->expandFoldSegment(parVA);
    mCodeFoldingManager->expandFoldSegment(parCIP);
  }
  // mCipVa = parCIP;
  if (mIsMain || !mMemPage->getBase()) disassembleAt(parVA, true, -1);
}

void DisassemblyFile::disassembleClear() {
  mHighlightingMode = false;
  mHighlightToken = ZydisTokenizer::SingleToken();
  historyClear();
  mManaDB = nullptr;
...
void DisassemblyFile::debugStateChangedSlot(DBGSTATE state) {
  switch (state) {
    case stopped:
      disassembleClear();
      break;
...
const duint DisassemblyFile::getBase() const { return mMemPage->getBase(); }

duint DisassemblyFile::getSize() const { return mMemPage->getSize(); }

duint DisassemblyFile::getTableOffsetRva() const {
  return mInstBuffer.size() ? mInstBuffer.at(0).rva : 0;
}

void DisassemblyFile::historyClear() {
  mVaHistory.clear();  // clear history for new targets
  mCurrentVa = 0;
}

void DisassemblyFile::historyPrevious() {
...
void DisassemblyFile::historyNext() {
...
bool DisassemblyFile::historyHasPrevious() const {
  if (!mCurrentVa || !mVaHistory.size())  // we are at the earliest history
                                          // entry
    return false;
  return true;
}

bool DisassemblyFile::historyHasNext() const {
  int size = mVaHistory.size();
  if (!size || mCurrentVa >=
                   mVaHistory.size() - 1)  // we are at the newest history entry
    return false;
  return true;
}

QString DisassemblyFile::getAddrText(dsint cur_addr, char label[MAX_LABEL_SIZE],
                                     bool getLabel) {
  QString addrText = "";
  ...
  if (getLabel && DbgGetLabelAt(cur_addr, SEG_DEFAULT, label_))  // has label
  {
    ...
    if (DbgGetModuleAt(cur_addr, module) &&
        !QString(label_).startsWith("JMP.&") && !mNoCurrentModuleText)
 ...
 */
void DisassemblyFile::setCodeFoldingManager(
    CodeFoldingHelper* CodeFoldingManager) {
 ...
 */
void DisassemblyFile::unfold(dsint rva) {
  if (mCodeFoldingManager) {
...
bool DisassemblyFile::hightlightToken(
    const ZydisTokenizer::SingleToken& token) {
  mHighlightToken = token;
  mHighlightingMode = false;
  return true;
}

bool DisassemblyFile::isHighlightMode() const { return mHighlightingMode; }

void DisassemblyFile::setTableOffset(dsint val) {
  mTableOffset = val;
