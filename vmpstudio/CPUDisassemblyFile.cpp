...
CPUDisassemblyFile::CPUDisassemblyFile(QWidget* parent, bool isMain)
    : DisassemblyFile(parent, isMain) {
...
void CPUDisassemblyFile::mousePressEvent(QMouseEvent* event) {
  if (event->buttons() == Qt::MiddleButton)  // copy address to clipboard
  {
    ...
    DisassemblyFile::mousePressEvent(event);
...
void CPUDisassemblyFile::mouseDoubleClickEvent(QMouseEvent* event) {
  ...
  switch (getColumnIndexFromX(event->x())) {
    ...
    case 2: {
      ...
      if (found != mDstRvas.end()) {
        gotoAddress(rvaToVa(found->second));
      }
      break;
    }
    ...
    // Undefined area
    default:
      DisassemblyFile::mouseDoubleClickEvent(event);
      break;
  }
}

void CPUDisassemblyFile::addFollowReferenceMenuItem(QString name, dsint value,
                                                    QMenu* menu,
                                                    bool isReferences,
                                                    bool isFollowInCPU) {
...
void CPUDisassemblyFile::setupFollowReferenceMenu(dsint wVA, QMenu* menu,
                                                  bool isReferences,
                                                  bool isFollowInCPU) {
 ...
 * @return      Nothing.
 */
void CPUDisassemblyFile::contextMenuEvent(QContextMenuEvent* event) {
  QMenu wMenu(this);
...
************************************************************************************/
void CPUDisassemblyFile::setupRightClickContextMenu() {
  ...
  mBreakpointMenu = new BreakpointMenu(this, getActionHelperFuncs(), [this]() {
    return false;  // rvaToVa(getInitialSelection());
  });
  ...
  analysisMenu->addAction(makeShortcutAction(
      DIcon("analysis_single_function.png"), tr("Analyze single function"),
      SLOT(analyzeSingleFunctionSlot()), "ActionAnalyzeSingleFunction"));
  ...
  const char* strTable[] = {
      "Code",  "Byte",  "Word",    "Dword", "Fword",  "Qword",
      "Tbyte", "Oword", nullptr,   "Float", "Double", "Long Double",
      nullptr, "ASCII", "UNICODE", nullptr,
      // "MMWord", "XMMWord", "YMMWord"
  };

  const char* shortcutTable[] = {
      "Code",  "Byte",  "Word",    "Dword", "Fword",  "Qword",
      "Tbyte", "Oword", nullptr,   "Float", "Double", "LongDouble",
      nullptr, "ASCII", "UNICODE", nullptr,
      //    "MMWord", "XMMWord", "YMMWord"
  };

  const char* iconTable[] = {
      "cmd",   "byte",  "word",    "dword", "fword",  "qword",
      "tbyte", "oword", nullptr,   "float", "double", "longdouble",
      nullptr, "ascii", "unicode", nullptr,
      // "mmword", "xmm", "ymm"
  };

  ENCODETYPE enctypeTable[] = {
      enc_code,   enc_byte,  enc_word,    enc_dword,  enc_fword, enc_qword,
      enc_tbyte,  enc_oword, enc_middle,  enc_real4,  enc_real8, enc_real10,
...
#endif
  mMenuBuilder->addMenu(
      makeMenu(DIcon("breakpoint_edit_alt.png"), tr("Edit Pro")), analysisMenu);
  ...
  mMenuBuilder->addAction(
      makeShortcutAction(DIcon("compile-error.png"), tr("Assemble"),
  ...
  mMenuBuilder->addAction(
      makeShortcutAction(DIcon("compile.png"), tr("Dynamic Decompile"),
                         SLOT(dynamicDecompileSlot()), "ActionDynDecompile"));
  ...
  mMenuBuilder->addAction(
    ...
    {
        return false;//DbgFunctions()->ModBaseFromAddr(rvaToVa(getInitialSelection())) != 0;
    });
...
void CPUDisassemblyFile::gotoOriginSlot() {
  if (!DbgIsDebugging()) return;
  gotoAddress(DbgValFromString("cip"));
}

void CPUDisassemblyFile::setNewOriginHereActionSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::setLabelSlot() {
  if (!DbgIsDebugging()) return;
  duint wVA = rvaToVa(getInitialSelection());
...
void CPUDisassemblyFile::setLabelAddressSlot() {
  if (!DbgIsDebugging()) return;
  BASIC_INSTRUCTION_INFO instr_info;
  DbgDisasmFastAt(rvaToVa(getInitialSelection()), &instr_info);
...
void CPUDisassemblyFile::setCommentSlot() {
  if (!DbgIsDebugging()) return;
  duint wVA = rvaToVa(getInitialSelection());
  LineEditDialog mLineEdit(this);
  ...
  if (DbgGetCommentAt((ManaDatabase*)mManaDB, (duint)wVA, comment_text)) {
  ...
  if (!DbgSetCommentAt((ManaDatabase*)mManaDB, wVA,
...
void CPUDisassemblyFile::setBookmarkSlot() {
  if (!DbgIsDebugging()) return;
  duint wVA = rvaToVa(getInitialSelection());
  bool result;
  if (DbgGetBookmarkAt((ManaDatabase*)mManaDB, wVA))
    result = DbgSetBookmarkAt((ManaDatabase*)mManaDB, wVA, false);
  else
    result = DbgSetBookmarkAt((ManaDatabase*)mManaDB, wVA, true);
  if (!result) {
...
void CPUDisassemblyFile::toggleFunctionSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::toggleArgumentSlot() {
  if (!DbgIsDebugging()) return;
  duint start = rvaToVa(getSelectionStart());
  duint end = rvaToVa(getSelectionEnd());
...
void CPUDisassemblyFile::addLoopSlot() {
  if (!DbgIsDebugging()) return;
  duint start = rvaToVa(getSelectionStart());
  duint end = rvaToVa(getSelectionEnd());
  if (start == end) return;
...
void CPUDisassemblyFile::deleteLoopSlot() {
  if (!DbgIsDebugging()) return;
  duint start = rvaToVa(getSelectionStart());
  auto depth = findDeepestLoopDepth(start);
  if (depth < 0) return;
...
void CPUDisassemblyFile::decompileSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::dynamicDecompileSlot() {
  if (!DbgIsDebugging()) return;

  dsint wRVA = getInitialSelection();
  DbgCmdExec(QString("urdyndec %1").arg(ToPtrString(rvaToVa(wRVA))));
}

void CPUDisassemblyFile::assembleSlot() {
  if (!DbgIsDebugging()) return;
...
#if VMP_STUDIO
      ManaFunc curFunc;
      if (mManaDB->db->archType() == mana::ARM) {
        auto fn = mManaDB->rvaToFunction(wRVA, false);
        curFunc.info.thumb = fn.thumb();
      }
      auto* diser = DbgGlobal::inst()->diser(curFunc);
#else
...
void CPUDisassemblyFile::gotoExpressionSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::gotoFileOffsetSlot() {
  if (!DbgIsDebugging()) return;
  char modname[MAX_MODULE_SIZE] = "";
...
void CPUDisassemblyFile::gotoNextReferenceSlot() {
  auto count = DbgEval("refsearch.count()"),
       index = DbgEval("$__disasm_refindex"),
       addr = DbgEval("refsearch.addr($__disasm_refindex)");
  if (count) {
...
void CPUDisassemblyFile::gotoXrefSlot() {
...
void CPUDisassemblyFile::followActionSlot() {
...
void CPUDisassemblyFile::gotoPreviousSlot() { historyPrevious(); }

void CPUDisassemblyFile::gotoNextSlot() { historyNext(); }

void CPUDisassemblyFile::findReferencesSlot() {
...
void CPUDisassemblyFile::findConstantSlot() {
  int refFindType = 0;
...
void CPUDisassemblyFile::findStringsSlot() {
  int refFindType = 0;
...
void CPUDisassemblyFile::findCallsSlot() {
  int refFindType = 0;
...
void CPUDisassemblyFile::findPatternSlot() {
  ...
  if (sender() == mFindPatternModule) {
    ...
    if (base)
      command = QString("findallmem %1, %2, %3")
                    .arg(ToHexString(base), hexEdit.mHexEdit->pattern(),
                         ToHexString(DbgFunctions()->ModSizeFromAddr(base)));
  }
...
void CPUDisassemblyFile::findGUIDSlot() {
  int refFindType = 0;
...
void CPUDisassemblyFile::findNamesSlot() {
...
void CPUDisassemblyFile::selectionGetSlot(SELECTIONDATA* selection) {
  selection->start = rvaToVa(getSelectionStart());
  selection->end = rvaToVa(getSelectionEnd());
  Bridge::getBridge()->setResult(BridgeResult::SelectionGet, 1);
}

void CPUDisassemblyFile::selectionSetSlot(const SELECTIONDATA* selection) {
...
void CPUDisassemblyFile::selectionUpdatedSlot() {
...
void CPUDisassemblyFile::enableHighlightingModeSlot() {
...
void CPUDisassemblyFile::binaryEditSlot() {
  HexEditDialog hexEdit(this);
...
void CPUDisassemblyFile::binaryFillSlot() {
  HexEditDialog hexEdit(this);
...
void CPUDisassemblyFile::binaryFillNopsSlot() {
  HexEditDialog hexEdit(this);
  dsint selStart = getSelectionStart();
  dsint selSize = getSelectionEnd() - selStart + 1;
  ...
  if (mManaDB->db->archType() == mana::ARM) {
    curFunc.info.thumb =
        mManaDB->rvaToFunction(vaToRva(selStart), false).thumb();
  }
  auto* diser = DbgGlobal::inst()->diser(curFunc);
#else
  auto* diser = DbgGlobal::inst()->diser(nullptr);
#endif
  diser->assemble("nop", nop);
...
void CPUDisassemblyFile::binaryCopySlot() {
  HexEditDialog hexEdit(this);
  dsint selStart = getSelectionStart();
  dsint selSize = getSelectionEnd() - selStart + 1;
  byte_t* data = new byte_t[selSize];
...
void CPUDisassemblyFile::binaryPasteSlot() {
...
void CPUDisassemblyFile::undoSelectionSlot() {
  dsint start = rvaToVa(getSelectionStart());
  dsint end = rvaToVa(getSelectionEnd());
...
void CPUDisassemblyFile::binaryPasteIgnoreSizeSlot() {
  if (!QApplication::clipboard()->mimeData()->hasText()) return;
  HexEditDialog hexEdit(this);
  dsint selStart = getSelectionStart();
  dsint selSize = getSelectionEnd() - selStart + 1;
...
void CPUDisassemblyFile::showPatchesSlot() { emit showPatches(); }

void CPUDisassemblyFile::copySelectionSlot(bool copyBytes) {
...
void CPUDisassemblyFile::copySelectionToFileSlot(bool copyBytes) {
...
void CPUDisassemblyFile::setSideBar(CPUSideBar* sideBar) {
  // mSideBar = sideBar;
}

void CPUDisassemblyFile::pushSelectionInto(bool copyBytes, QTextStream& stream,
...
void CPUDisassemblyFile::copySelectionSlot() { copySelectionSlot(true); }

void CPUDisassemblyFile::copySelectionToFileSlot() {
  copySelectionToFileSlot(true);
}

void CPUDisassemblyFile::copySelectionNoBytesSlot() {
  copySelectionSlot(false);
}

void CPUDisassemblyFile::copySelectionToFileNoBytesSlot() {
  copySelectionToFileSlot(false);
}

void CPUDisassemblyFile::copyAddressSlot() {
  QString clipboard = "";
  prepareDataRange(getSelectionStart(), getSelectionEnd(),
                   [&](int i, const Instruction_t& inst) {
                     if (i) clipboard += "\r\n";
...
void CPUDisassemblyFile::copyRvaSlot() {
  QString clipboard = "";
  prepareDataRange(
      getSelectionStart(), getSelectionEnd(),
      [&](int i, const Instruction_t& inst) {
        if (i) clipboard += "\r\n";
        duint addr = rvaToVa(inst.rva);
        duint base = mManaDB->rtbase;  // DbgFunctions()->ModBaseFromAddr(addr);
        if (1 || base)
...
void CPUDisassemblyFile::copyFileOffsetSlot() {
  QString clipboard = "";
  prepareDataRange(
      getSelectionStart(), getSelectionEnd(),
      [&](int i, const Instruction_t& inst) {
        if (i) clipboard += "\r\n";
        auto sect = mManaDB->rvaToSection(inst.rva);

        duint offset =
            sect->foff + (inst.rva - (sect->addr - mManaDB->db->imageBase()));
        if (1 || offset)
...
void CPUDisassemblyFile::copyHeaderVaSlot() {
  QString clipboard = "";
  prepareDataRange(
      getSelectionStart(), getSelectionEnd(),
      [&](int i, const Instruction_t& inst) {
        ...
        if (1 || base) {
          ...
          clipboard += ToPtrString(
              base);  // ToPtrString(DbgValFromString(expr.toUtf8().constData()));
        } else {
          SimpleWarningBox(this, tr("Error!"),
                           tr("Selection not in a module..."));
          return false;
...
void CPUDisassemblyFile::copyDisassemblySlot() {
...
void CPUDisassemblyFile::labelCopySlot() {
  QString symbol = ((QAction*)sender())->text();
  Bridge::CopyToClipboard(symbol);
}

void CPUDisassemblyFile::findCommandSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::openSourceSlot() {
...
void CPUDisassemblyFile::displayWarningSlot(QString title, QString text) {
  SimpleWarningBox(this, title, text);
}

void CPUDisassemblyFile::paintEvent(QPaintEvent* event) {
  ...
  DisassemblyFile::paintEvent(event);
}

int CPUDisassemblyFile::findDeepestLoopDepth(duint addr) {
  for (int depth = 0;; depth++)
    if (!DbgLoopGet(depth, addr, nullptr, nullptr)) return depth - 1;
  return -1;  // unreachable
}

bool CPUDisassemblyFile::getLabelsFromInstruction(duint addr,
                                                  QSet<QString>& labels) {
...
void CPUDisassemblyFile::labelHelpSlot() {
...
void CPUDisassemblyFile::ActionTraceRecordBitSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::ActionTraceRecordByteSlot() {
  if (!DbgIsDebugging()) return;
  duint base = mMemPage->getBase();
  duint size = mMemPage->getSize();
  for (duint i = base; i < base + size; i += 4096) {
...
void CPUDisassemblyFile::ActionTraceRecordWordSlot() {
  if (!DbgIsDebugging()) return;
  duint base = mMemPage->getBase();
  duint size = mMemPage->getSize();
  for (duint i = base; i < base + size; i += 4096) {
...
void CPUDisassemblyFile::ActionTraceRecordDisableSlot() {
  if (!DbgIsDebugging()) return;
  duint base = mMemPage->getBase();
  duint size = mMemPage->getSize();
  for (duint i = base; i < base + size; i += 4096) {
...
void CPUDisassemblyFile::mnemonicBriefSlot() {
  mShowMnemonicBrief = !mShowMnemonicBrief;
  reloadData();
}

void CPUDisassemblyFile::mnemonicHelpSlot() {
...
void CPUDisassemblyFile::analyzeSingleFunctionSlot() {
  DbgCmdExec(
      QString("analr %1").arg(ToHexString(rvaToVa(getInitialSelection()))));
}

void CPUDisassemblyFile::removeAnalysisSelectionSlot() {
  if (!DbgIsDebugging()) return;
  WordEditDialog mLineEdit(this);
...
void CPUDisassemblyFile::removeAnalysisModuleSlot() {
  if (!DbgIsDebugging()) return;
  mDisasm->getEncodeMap()->delSegment(rvaToVa(getSelectionStart()));
  GuiUpdateDisassemblyView();
}

void CPUDisassemblyFile::setEncodeTypeRangeSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDisassemblyFile::setEncodeTypeSlot() {
  if (!DbgIsDebugging()) return;
  QAction* pAction = qobject_cast<QAction*>(sender());
  mDisasm->getEncodeMap()->setDataType(rvaToVa(getSelectionStart()),
                                       (ENCODETYPE)pAction->data().toUInt());
...
void CPUDisassemblyFile::graphSlot() {
...
void CPUDisassemblyFile::analyzeModuleSlot() {
  DbgCmdExec("cfanal");
  DbgCmdExec("analx");
}

void CPUDisassemblyFile::createThreadSlot() {
...
void CPUDisassemblyFile::copyTokenTextSlot() {
  Bridge::CopyToClipboard(mHighlightToken.text);
}

void CPUDisassemblyFile::copyTokenValueSlot() {
  QString text;
  if (getTokenValueText(text)) Bridge::CopyToClipboard(text);
}

bool CPUDisassemblyFile::getTokenValueText(QString& text) {
...
void CPUDisassemblyFile::analyzeFinishedSlot() {
  if (mAnaThread) {
    delete mAnaThread;
    mAnaThread = nullptr;
  }

  unsigned fcrc = DbgGlobal::inst()->pathCRC(mPath);
  auto found = DbgGlobal::inst()->modules.find(fcrc);
  if (found == DbgGlobal::inst()->modules.end()) {
    return;
  }
  mManaDB = found->second;
  mMemPage->setAttributes(mManaDB->rtbase, mManaDB->opcsize, mManaDB);

  disassembleAt(0, false, -1);
  show();
  setFocus();
}

void CPUDisassemblyFile::resetDatabase(VMPManaDatabase* mdb) {
  if (!mdb) {
    mManaDB = nullptr;
    reloadData();
    return;
  ...
  int totalsz = 0;
  for (auto& s : mdb->db->sections()) {
    auto sect = &s.second;
    if (sect->type == mana::TEXT) {
      int fileoff = sect->foff + sect->size;
      if (totalsz < fileoff) {
        totalsz = fileoff;
      }
    }
  ...
  mManaDB = mdb;
  mMemPage->setAttributes(mManaDB->rtbase, totalsz, mManaDB);
  ...
  disassembleAt(addr, false, -1);
  reloadData();
}

void CPUDisassemblyFile::editFunSlot() {
  auto debugee = VMPGlobal::inst()->debugee;
  auto func = debugee->rvaToFunction(getInitialSelection(), false);
  if (!func) return;
  EditFunctionDialog dlg(this, &func);
  dlg.exec();
}
