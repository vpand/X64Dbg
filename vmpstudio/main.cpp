...
int main(int argc, char* argv[]) {
  install_breakpad(VMPGlobal::inst()->breakpadDir().toUtf8().data(), "VMPStudio", STUDIO_VERSION);
  ...
  QFileInfo cfgfinfo(VMPGlobal::inst()->cfgPath());
  if (cfgfinfo.exists()) {
  ...
  mainWindow->show();
  check_crash_dump(VMPGlobal::inst()->breakpadDir());
