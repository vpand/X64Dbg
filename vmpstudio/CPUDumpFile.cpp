...
CPUDumpFile::CPUDumpFile(CPUDisassemblyFile* disas, CPUMultiDumpFile* multiDump,
                         QWidget* parent)
    : HexDumpFile(parent) {
  mDisas = disas;
  ...
  connect(this, SIGNAL(headerButtonReleased(int)), this,
          SLOT(headerButtonReleasedSlot(int)));

  mPluginMenu = nullptr;  // multiDump->mDumpPluginMenu;
...
void CPUDumpFile::setupContextMenu() {
  ...
  wBinaryMenu->addAction(
      ...
      [](QMenu*) { return QApplication::clipboard()->mimeData()->hasText(); });
  wBinaryMenu->addAction(
      makeShortcutAction(DIcon("binary_save.png"), tr("Save To a File"),
                         SLOT(binarySaveToFileSlot()), "ActionBinarySave"));
  mMenuBuilder->addMenu(makeMenu(DIcon("binary.png"), tr("B&inary")),
                        wBinaryMenu);
  ...
  wCopyMenu->addAction(mCopyRva, [this](QMenu*) {
    return true;  // DbgFunctions()->ModBaseFromAddr(rvaToVa(getInitialSelection()))
                  // != 0;
  });
  wCopyMenu->addAction(
      ...
      [this](QMenu*) {
        return true;  // DbgFunctions()->VaToFileOffset(rvaToVa(getInitialSelection()))
                      // != 0;
      });
  ...
  mMenuBuilder->addAction(
      ...
      [this](QMenu*) {
        return false;  // DbgFunctions()->PatchInRange(rvaToVa(getSelectionStart()),
                       // rvaToVa(getSelectionEnd()));
      });

  mMenuBuilder->addAction(makeShortcutAction(
      DIcon(ArchValue("processor32.png", "processor-cpu.png")),
      tr("Follow in Disassembler"), SLOT(followInDisasmSlot()),
      "ActionFollowDisasm"));
  auto wIsValidReadPtrCallback = [this](QMenu*) {
    duint ptr = 0;
    DbgMemRead(rvaToVa(getSelectionStart()), (unsigned char*)&ptr,
               sizeof(duint));
  ...
  mMenuBuilder->addAction(
      makeShortcutAction(
          DIcon(ArchValue("processor32.png", "processor-cpu.png")),
          ArchValue(tr("&Follow DWORD in Disassembler"),
                    tr("&Follow PTR in Disassembler")),
          SLOT(followDataSlot()), "ActionFollowDwordQwordDisasm"),
      wIsValidReadPtrCallback);
  mMenuBuilder->addAction(
      makeShortcutAction(DIcon("dump.png"),
                         ArchValue(tr("&Follow DWORD in Current Dump"),
                                   tr("&Follow PTR in Current Dump")),
                         SLOT(followDataDumpSlot()),
                         "ActionFollowDwordQwordDump"),
      wIsValidReadPtrCallback);

  MenuBuilder* wFollowInDumpMenu =
      new MenuBuilder(this, [wIsValidReadPtrCallback, this](QMenu* menu) {
        if (!wIsValidReadPtrCallback(menu)) return false;
  ...
  for (int i = 0; i < maxDumps; i++) {
    QAction* action =
        makeAction(DIcon("dump.png"), QString(), SLOT(followInDumpNSlot()));
    wFollowInDumpMenu->addAction(action);
    mFollowInDumpActions.push_back(action);
  }
  mMenuBuilder->addMenu(
      makeMenu(DIcon("dump.png"), ArchValue(tr("&Follow DWORD in Dump"),
                                            tr("&Follow PTR in Dump"))),
      wFollowInDumpMenu);
  ...
  wHardwareAccessMenu->addAction(makeAction(
      DIcon("breakpoint_byte.png"), tr("&Byte"), SLOT(hardwareAccess1Slot())));
  wHardwareAccessMenu->addAction(makeAction(
      DIcon("breakpoint_word.png"), tr("&Word"), SLOT(hardwareAccess2Slot())));
  wHardwareAccessMenu->addAction(makeAction(DIcon("breakpoint_dword.png"),
                                            tr("&Dword"),
                                            SLOT(hardwareAccess4Slot())));
#ifdef _WIN64
  wHardwareAccessMenu->addAction(makeAction(DIcon("breakpoint_qword.png"),
                                            tr("&Qword"),
                                            SLOT(hardwareAccess8Slot())));
#endif  //_WIN64
  wHardwareWriteMenu->addAction(makeAction(
...
#ifdef _WIN64
  wHardwareWriteMenu->addAction(makeAction(
      DIcon("breakpoint_qword.png"), tr("&Qword"), SLOT(hardwareWrite8Slot())));
#endif  //_WIN64
  ...
  wGotoMenu->addAction(
      makeShortcutAction(DIcon("top.png"), tr("Start of Page"),
                         SLOT(gotoStartSlot()), "ActionGotoStart"),
      [this](QMenu*) { return getSelectionStart() != 0; });
  wGotoMenu->addAction(
      makeShortcutAction(DIcon("bottom.png"), tr("End of Page"),
  ...
  wGotoMenu->addAction(
      ...
      [](QMenu*) {
        return !!DbgEval(
            "refsearch.count() && ($__dump_refindex > 0 || dump.sel() != "
            "refsearch.addr($__dump_refindex))");
      });
  wGotoMenu->addAction(
      ...
      [](QMenu*) {
        return !!DbgEval(
            "refsearch.count() && ($__dump_refindex < refsearch.count() || "
            "dump.sel() != refsearch.addr($__dump_refindex))");
      });
  ...
  wHexMenu->addAction(
      makeAction(DIcon("ascii.png"), tr("&ASCII"), SLOT(hexAsciiSlot())));
  wHexMenu->addAction(makeAction(DIcon("ascii-extended.png"),
                                 tr("&Extended ASCII"),
                                 SLOT(hexUnicodeSlot())));
  QAction* wHexLastCodepage =
      makeAction(DIcon("codepage.png"), "?", SLOT(hexLastCodepageSlot()));
  wHexMenu->addAction(wHexLastCodepage, [wHexLastCodepage](QMenu*) {
    duint lastCodepage;
    auto allCodecs = QTextCodec::availableCodecs();
    if (!BridgeSettingGetUint("Misc", "LastCodepage", &lastCodepage) ||
        lastCodepage >= duint(allCodecs.size()))
      return false;
    wHexLastCodepage->setText(
        QString::fromLocal8Bit(allCodecs.at(lastCodepage)));
    return true;
  });
  wHexMenu->addAction(makeAction(DIcon("codepage.png"), tr("&Codepage..."),
                                 SLOT(hexCodepageSlot())));
  mMenuBuilder->addMenu(makeMenu(DIcon("hex.png"), tr("&Hex")), wHexMenu);

  MenuBuilder* wTextMenu = new MenuBuilder(this);
  wTextMenu->addAction(
      makeAction(DIcon("ascii.png"), tr("&ASCII"), SLOT(textAsciiSlot())));
  wTextMenu->addAction(makeAction(DIcon("ascii-extended.png"),
                                  tr("&Extended ASCII"),
                                  SLOT(textUnicodeSlot())));
  QAction* wTextLastCodepage =
      makeAction(DIcon("codepage.png"), "?", SLOT(textLastCodepageSlot()));
  wTextMenu->addAction(wTextLastCodepage, [wTextLastCodepage](QMenu*) {
    ...
    if (!BridgeSettingGetUint("Misc", "LastCodepage", &lastCodepage) ||
        lastCodepage >= duint(allCodecs.size()))
      return false;
    wTextLastCodepage->setText(
        QString::fromLocal8Bit(allCodecs.at(lastCodepage)));
    return true;
  });
  wTextMenu->addAction(makeAction(DIcon("codepage.png"), tr("&Codepage..."),
                                  SLOT(textCodepageSlot())));
  mMenuBuilder->addMenu(makeMenu(DIcon("strings.png"), tr("&Text")), wTextMenu);

  MenuBuilder* wIntegerMenu = new MenuBuilder(this);
  wIntegerMenu->addAction(makeAction(DIcon("byte.png"),
                                     tr("Signed byte (8-bit)"),
                                     SLOT(integerSignedByteSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("word.png"),
                                     tr("Signed short (16-bit)"),
                                     SLOT(integerSignedShortSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("dword.png"),
                                     tr("Signed long (32-bit)"),
                                     SLOT(integerSignedLongSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("qword.png"),
                                     tr("Signed long long (64-bit)"),
                                     SLOT(integerSignedLongLongSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("byte.png"),
                                     tr("Unsigned byte (8-bit)"),
                                     SLOT(integerUnsignedByteSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("word.png"),
                                     tr("Unsigned short (16-bit)"),
                                     SLOT(integerUnsignedShortSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("dword.png"),
                                     tr("Unsigned long (32-bit)"),
                                     SLOT(integerUnsignedLongSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("qword.png"),
                                     tr("Unsigned long long (64-bit)"),
                                     SLOT(integerUnsignedLongLongSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("word.png"),
                                     tr("Hex short (16-bit)"),
                                     SLOT(integerHexShortSlot())));
  wIntegerMenu->addAction(makeAction(
      DIcon("dword.png"), tr("Hex long (32-bit)"), SLOT(integerHexLongSlot())));
  wIntegerMenu->addAction(makeAction(DIcon("qword.png"),
                                     tr("Hex long long (64-bit)"),
                                     SLOT(integerHexLongLongSlot())));
  mMenuBuilder->addMenu(makeMenu(DIcon("integer.png"), tr("&Integer")),
                        wIntegerMenu);

  MenuBuilder* wFloatMenu = new MenuBuilder(this);
  wFloatMenu->addAction(makeAction(
      DIcon("32bit-float.png"), tr("&Float (32-bit)"), SLOT(floatFloatSlot())));
  wFloatMenu->addAction(makeAction(DIcon("64bit-float.png"),
                                   tr("&Double (64-bit)"),
                                   SLOT(floatDoubleSlot())));
  wFloatMenu->addAction(makeAction(DIcon("80bit-float.png"),
                                   tr("&Long double (80-bit)"),
                                   SLOT(floatLongDoubleSlot())));
  mMenuBuilder->addMenu(makeMenu(DIcon("float.png"), tr("&Float")), wFloatMenu);

  mMenuBuilder->addAction(makeAction(DIcon("address.png"), tr("&Address"),
                                     SLOT(addressAsciiSlot())));
  mMenuBuilder->addAction(makeAction(
      DIcon("processor-cpu.png"), tr("&Disassembly"), SLOT(disassemblySlot())));
...
void CPUDumpFile::getAttention() {
  BackgroundFlickerThread* thread =
      new BackgroundFlickerThread(this, mBackgroundColor, this);
  connect(thread, SIGNAL(finished()), thread, SLOT(deleteLater()));
  thread->start();
}

void CPUDumpFile::getColumnRichText(int col, dsint rva,
                                    RichTextPainter::List& richText) {
  if (col && !mDescriptor.at(col - 1).isData &&
      mDescriptor.at(col - 1).itemCount)  // print comments
  {
    ...
    if (!curData.text.length())  // stack comments
    {
      ...
      if (va >= stackBase && va < stackBase + stackSize &&
          DbgStackCommentGet(va, &comment)) {
    ...
    if (curData.text.length()) richText.push_back(curData);
  } else
    HexDumpFile::getColumnRichText(col, rva, richText);
}

QString CPUDumpFile::paintContent(QPainter* painter, dsint rowBase,
  ...
  if (!col)  // address
  {
    ...
    dsint cur_addr =
        rvaToVa((rowBase + rowOffset) * getBytePerRowCount() - mByteOffset);
    QColor background;
    ...
    if (background.alpha())
      painter->fillRect(QRect(x, y, w, h),
                        QBrush(background));  // fill background color
    painter->drawText(QRect(x + 4, y, w - 4, h),
                      Qt::AlignVCenter | Qt::AlignLeft, makeAddrText(cur_addr));
    return QString();
  }
  return HexDumpFile::paintContent(painter, rowBase, rowOffset, col, x, y, w,
                                   h);
}

void CPUDumpFile::contextMenuEvent(QContextMenuEvent* event) {
  QMenu wMenu(this);
  mMenuBuilder->build(&wMenu);
  wMenu.exec(event->globalPos());
}

void CPUDumpFile::mouseDoubleClickEvent(QMouseEvent* event) {
  if (event->button() != Qt::LeftButton || !DbgIsDebugging()) return;
  switch (getColumnIndexFromX(event->x())) {
    case 0:  // address
    {
      // very ugly way to calculate the base of the current row (no clue why it
      // works)
      dsint deltaRowBase =
          getInitialSelection() % getBytePerRowCount() + mByteOffset;
...
static QString getTooltipForVa(duint va, int depth) {
  duint ptr = 0;
  if (!DbgMemRead(va, &ptr, sizeof(duint))) return QString();
  ...
  switch (DbgGetEncodeTypeAt(va, 1)) {
    ...
    default:
      if (DbgMemIsValidReadPtr(ptr) && depth >= 0) {
        tooltip = QString("[%1] = %2")
                      .arg(ToPtrString(ptr), getTooltipForVa(ptr, depth - 1));
      }
      // If not a pointer, hide tooltips
        ...
        if (isCodePage) {
          ...
          tooltip =
              QString("[%1] = %2").arg(ToPtrString(va)).arg(ToPtrString(ptr));
        if (DbgFunctions()->ModGetParty(va) == 1)
          tooltip += " (" +
                     (isCodePage ? CPUDumpFile::tr("System Code")
                                 : CPUDumpFile::tr("System Data")) +
                     ")";
        else
          tooltip += " (" +
                     (isCodePage ? CPUDumpFile::tr("User Code")
                                 : CPUDumpFile::tr("User Data")) +
                     ")";
      }
      break;
    case enc_code:
      ...
      if (GuiGetDisassembly(va, disassembly))
        ...
        tooltip = "";
      if (DbgFunctions()->ModGetParty(va) == 1)
        tooltip += " (" + CPUDumpFile::tr("System Code") + ")";
      else
        tooltip += " (" + CPUDumpFile::tr("User Code") + ")";
      break;
    case enc_real4:
      tooltip = ToFloatString(&va) + CPUDumpFile::tr(" (Real4)");
      break;
    case enc_real8:
      double numd;
      DbgMemRead(va, &numd, sizeof(double));
      tooltip = ToDoubleString(&numd) + CPUDumpFile::tr(" (Real8)");
      break;
    case enc_byte:
      tooltip = ToByteString(va) + CPUDumpFile::tr(" (BYTE)");
      break;
    case enc_word:
      tooltip = ToWordString(va) + CPUDumpFile::tr(" (WORD)");
      break;
    case enc_dword:
      tooltip =
          QString("%1").arg((unsigned int)va, 8, 16, QChar('0')).toUpper() +
          CPUDumpFile::tr(" (DWORD)");
      break;
    case enc_qword:
#ifdef _WIN64
      tooltip = QString("%1")
                    .arg((unsigned long long)va, 16, 16, QChar('0'))
                    .toUpper() +
                CPUDumpFile::tr(" (QWORD)");
#else  // x86
    ...
    case enc_unicode:
      ...
      if (DbgGetStringAt(va, str))
        tooltip = QString::fromUtf8(str) + CPUDumpFile::tr(" (String)");
      else
        tooltip = CPUDumpFile::tr("(Unknown String)");
      break;
  }
  return tooltip;
}
...
#if 0
void CPUDumpFile::mouseMoveEvent(QMouseEvent* event) {
  ...
  HexDumpFile::mouseMoveEvent(event);
}
#endif

void CPUDumpFile::setLabelSlot() {
...
void CPUDumpFile::gotoFileOffsetSlot() {
  if (!mGotoOffset) mGotoOffset = new GotoDialog(this);
  mGotoOffset->fileOffset = true;
  const char* modname = "File";
  mGotoOffset->modName = QString(modname);
  mGotoOffset->setWindowTitle(
      tr("Goto File Offset in %1").arg(QString(modname)));
...
void CPUDumpFile::gotoStartSlot() {
  duint dest = mMemPage->getBase();
  DbgCmdExec(QString().sprintf("dump \"%p\"", dest));
}

void CPUDumpFile::gotoEndSlot() {
  duint dest = mMemPage->getBase() + mMemPage->getSize() -
               (getViewableRowsCount() * getBytePerRowCount());
  DbgCmdExec(QString().sprintf("dump \"%p\"", dest));
}

void CPUDumpFile::gotoPreviousReferenceSlot() {
  auto count = DbgEval("refsearch.count()"),
       index = DbgEval("$__dump_refindex"),
       addr = DbgEval("refsearch.addr($__dump_refindex)");
  if (count) {
...
void CPUDumpFile::gotoNextReferenceSlot() {
  auto count = DbgEval("refsearch.count()"),
       index = DbgEval("$__dump_refindex"),
       addr = DbgEval("refsearch.addr($__dump_refindex)");
  if (count) {
...
void CPUDumpFile::hexUnicodeSlot() {
...
void CPUDumpFile::hexCodepageSlot() {
...
void CPUDumpFile::hexLastCodepageSlot() {
...
void CPUDumpFile::textLastCodepageSlot() {
...
void CPUDumpFile::textAsciiSlot() {
...
void CPUDumpFile::textUnicodeSlot() {
...
void CPUDumpFile::textCodepageSlot() {
  CodepageSelectionDialog dialog(this);
  if (dialog.exec() != QDialog::Accepted) return;
  auto codepage = dialog.getSelectedCodepage();
...
void CPUDumpFile::integerSignedByteSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 40, tr("Signed byte (8-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerSignedShortSlot() {
  ...
  wColDesc.data.wordMode = SignedDecWord;
  appendResetDescriptor(8 + charwidth * 55, tr("Signed short (16-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerSignedLongSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 47, tr("Signed long (32-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerSignedLongLongSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 41, tr("Signed long long (64-bit)"),
                        false, wColDesc);
...
void CPUDumpFile::integerUnsignedByteSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 32, tr("Unsigned byte (8-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerUnsignedShortSlot() {
  ...
  wColDesc.data.itemSize = Word;
  wColDesc.data.wordMode = UnsignedDecWord;
  appendResetDescriptor(8 + charwidth * 47, tr("Unsigned short (16-bit)"),
                        false, wColDesc);
...
void CPUDumpFile::integerUnsignedLongSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 43, tr("Unsigned long (32-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerUnsignedLongLongSlot() {
  Config()->setUint("HexDump", "DefaultView",
                    (duint)ViewIntegerUnsignedLongLong);
  int charwidth = getCharWidth();
  ColumnDescriptor wColDesc;
  DataDescriptor dDesc;
  ...
  appendResetDescriptor(8 + charwidth * 41, tr("Unsigned long long (64-bit)"),
                        false, wColDesc);
...
void CPUDumpFile::integerHexShortSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 39, tr("Hex short (16-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerHexLongSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 35, tr("Hex long (32-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::integerHexLongLongSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 33, tr("Hex long long (64-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::floatFloatSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 55, tr("Float (32-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::floatDoubleSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 47, tr("Double (64-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::floatLongDoubleSlot() {
  ...
  appendResetDescriptor(8 + charwidth * 59, tr("Long double (80-bit)"), false,
                        wColDesc);
...
void CPUDumpFile::addressAsciiSlot() {
  ...
  wColDesc.data.dwordMode = HexDword;
#endif
  appendResetDescriptor(8 + charwidth * 2 * sizeof(duint), tr("Value"), false,
                        wColDesc);
  ...
  wColDesc.columnSwitch = [this]() { this->setView(ViewAddressUnicode); };
  appendDescriptor(8 + charwidth * wColDesc.itemCount, tr("ASCII"), true,
                   wColDesc);
...
void CPUDumpFile::addressUnicodeSlot() {
  ...
  wColDesc.columnSwitch = [this]() { this->setView(ViewAddressAscii); };
  appendDescriptor(8 + charwidth * wColDesc.itemCount, tr("UNICODE"), true,
                   wColDesc);
...
void CPUDumpFile::disassemblySlot() {
  SELECTIONDATA selection;
  selectionGet(&selection);
  emit showDisassemblyTab(selection.start, selection.end,
                          rvaToVa(getTableOffsetRva()));
}

void CPUDumpFile::selectionGet(SELECTIONDATA* selection) {
  selection->start = rvaToVa(getSelectionStart());
  selection->end = rvaToVa(getSelectionEnd());
  Bridge::getBridge()->setResult(BridgeResult::SelectionGet, 1);
}

void CPUDumpFile::selectionSet(const SELECTIONDATA* selection) {
...
void CPUDumpFile::binaryEditSlot() {
  ...
  hexEdit.setWindowTitle(
      tr("Edit data at %1").arg(ToPtrString(rvaToVa(selStart))));
...
void CPUDumpFile::binaryFillSlot() {
  HexEditDialog hexEdit(this);
  hexEdit.showKeepSize(false);
  hexEdit.mHexEdit->setOverwriteMode(false);
  dsint selStart = getSelectionStart();
  hexEdit.setWindowTitle(
      tr("Fill data at %1").arg(ToPtrString(rvaToVa(selStart))));
  if (hexEdit.exec() != QDialog::Accepted) return;
...
void CPUDumpFile::binaryCopySlot() {
  HexEditDialog hexEdit(this);
  dsint selStart = getSelectionStart();
  dsint selSize = getSelectionEnd() - selStart + 1;
  byte_t* data = new byte_t[selSize];
...
void CPUDumpFile::binaryPasteSlot() {
...
void CPUDumpFile::binaryPasteIgnoreSizeSlot() {
  if (!QApplication::clipboard()->mimeData()->hasText()) return;
  HexEditDialog hexEdit(this);
  dsint selStart = getSelectionStart();
  dsint selSize = getSelectionEnd() - selStart + 1;
...
void CPUDumpFile::binarySaveToFileSlot() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save to file"), QDir::currentPath(), tr("All files (*.*)"));
  if (fileName.length()) {
    ...
    QString cmd = QString("savedata \"%1\",%2,%3")
                      .arg(fileName, ToHexString(rvaToVa(selStart)),
                           ToHexString(selSize));
    DbgCmdExec(cmd);
  }
}

void CPUDumpFile::findPattern() {
  HexEditDialog hexEdit(this);
  ...
  DbgCmdExec(QString("findall " + addrText + ", " +
                     hexEdit.mHexEdit->pattern() + ", &data&"));
  emit displayReferencesWidget();
}

void CPUDumpFile::copyFileOffsetSlot() {
  duint addr = rvaToVa(getInitialSelection());
...
void CPUDumpFile::undoSelectionSlot() {
...
void CPUDumpFile::syncWithExpressionSlot() {
  if (!DbgIsDebugging()) return;
...
void CPUDumpFile::followInDumpNSlot() {
  for (int i = 0; i < mFollowInDumpActions.length(); i++)
    if (mFollowInDumpActions[i] == sender())
      DbgCmdExec(QString("dump [%1], .%2")
                     .arg(ToPtrString(rvaToVa(getSelectionStart())))
                     .arg(i + 1));
}

void CPUDumpFile::watchSlot() {
  DbgCmdExec(QString("AddWatch %1, \"uint\"")
                 .arg(ToPtrString(rvaToVa(getSelectionStart()))));
}

void CPUDumpFile::allocMemorySlot() {
  ...
  if (mLineEdit.exec() == QDialog::Accepted) {
    ...
    if (memsize == 0)  // 1GB
    {
      SimpleWarningBox(
          this, tr("Warning"),
          tr("You're trying to allocate a zero-sized buffer just now."));
      return;
    }
    if (memsize > 1024 * 1024 * 1024) {
      SimpleErrorBox(
          this, tr("Error"),
          tr("The size of buffer you're trying to allocate exceeds 1GB. Please "
             "check your expression to ensure nothing is wrong."));
      return;
    }
...
void CPUDumpFile::setView(ViewEnum_t view) {
  switch (view) {
    case ViewHexAscii:
      hexAsciiSlot();
      break;
...
void CPUDumpFile::followInMemoryMapSlot() {
  DbgCmdExec(
      QString("memmapdump %1").arg(ToHexString(rvaToVa(getSelectionStart()))));
}

void CPUDumpFile::headerButtonReleasedSlot(int colIndex) {
