#ifndef _HEXDUMPFILE_H
#define _HEXDUMPFILE_H
...
class HexDumpFile : public AbstractTableView {
  Q_OBJECT
 public:
  enum DataSize { Byte = 1, Word = 2, Dword = 4, Qword = 8, Tword = 10 };

  enum ByteViewMode { HexByte, AsciiByte, SignedDecByte, UnsignedDecByte };

  enum WordViewMode { HexWord, UnicodeWord, SignedDecWord, UnsignedDecWord };
  ...
  struct ColumnDescriptor {
    ...
    int itemCount = 16;
    int separator = 0;
    QTextCodec* textCodec = nullptr;  // name of the text codec (leave empty if
                                      // you want to keep your sanity)
    DataDescriptor data;
    std::function<void()> columnSwitch;
  };

  explicit HexDumpFile(QWidget* parent = 0);
  ~HexDumpFile() override;

  bool updateFile(const QString& path) { return mMemPage->open(path); }
  void updateMemPage(VMPOpcodePage* page) { mMemPage->setAttributes(page); }
  ...
  void printSelected(QPainter* painter, dsint rowBase, int rowOffset, int col,
  ...
  virtual void getColumnRichText(int col, dsint rva,
                                 RichTextPainter::List& richText);

  static int getSizeOf(DataSize size);

  void toString(DataDescriptor desc, duint rva, byte_t* data,
                RichTextPainter::CustomRichText_t& richText);

  void byteToString(duint rva, byte_t byte, ByteViewMode mode,
                    RichTextPainter::CustomRichText_t& richText);
  void wordToString(duint rva, uint16 word, WordViewMode mode,
                    RichTextPainter::CustomRichText_t& richText);
  static void dwordToString(duint rva, uint32 dword, DwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText);
  static void qwordToString(duint rva, uint64 qword, QwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText);
  static void twordToString(duint rva, void* tword, TwordViewMode mode,
                            RichTextPainter::CustomRichText_t& richText);
  ...
  void appendDescriptor(int width, QString title, bool clickable,
                        ColumnDescriptor descriptor);
  void appendResetDescriptor(int width, QString title, bool clickable,
                             ColumnDescriptor descriptor);
  void clearDescriptors();

  void printDumpAt(dsint parVA, bool select, bool repaint = true,
                   bool updateTableOffset = true);
 ...
 private:
  enum GuiState { NoState, MultiRowsSelectionState };
  ...
  struct UpdateCache {
    ...
    bool operator==(const UpdateCache& o) const {
      return std::tie(memBase, memSize, rva, size) ==
             std::tie(o.memBase, o.memSize, o.rva, o.size);
    }
  } mUpdateCache;
